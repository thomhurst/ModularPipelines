...
public abstract partial class CobraCliScraper : CliScraperBase
{
    ...
    public override async Task<CliToolDefinition> ScrapeAsync(CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation("Discovering {Tool} commands via CLI...", ToolName);

        // Check if tool is available
        if (!await IsAvailableAsync(cancellationToken))
        {
            Logger.LogError("{Tool} is not available on this system", ToolName);
            errors.Add(new ScrapingError
            {
                Source = ToolName,
                Message = $"{ToolName} is not available on this system. Ensure it is installed and in PATH."
            });
        ...
        try
        {
            ...
            var commandPaths = await DiscoverCommandsAsync([ToolName], cancellationToken);
            Logger.LogInformation("Discovered {Count} commands for {Tool}", commandPaths.Count, ToolName);
    ...
    private CliToolDefinition CreateToolDefinition(
