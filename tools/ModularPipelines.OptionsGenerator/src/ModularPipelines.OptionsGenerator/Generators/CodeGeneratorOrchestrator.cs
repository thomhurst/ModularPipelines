...
public class CodeGeneratorOrchestrator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var toolDefinition = await cliScraper.CreateToolDefinitionAsync(cancellationToken);
        ...
        ICollection<string> deletedPaths)
    {
        ...
        var namespacePrefixes = GetNamespacePrefixesForOutputDirectory(
            outputDirectory,
            toolOutputDirectory);
        ...
        foreach (var subDir in GeneratedCleanupDirectories)
        {
            ...
            foreach (var file in generatedFiles)
            {
                ...
                if (FileIsOwnedByNamespacePrefix(fileName, namespacePrefix, namespacePrefixes)
        ...
        ICollection<string> deletedPaths)
    {
        ...
        foreach (var subDir in GeneratedCleanupDirectories)
        {
            ...
            {
                if (FileIsOwnedByNamespacePrefix(
    ...
    private IReadOnlyList<string> GetNamespacePrefixesForOutputDirectory(
        string outputDirectory,
        string toolOutputDirectory)
    {
    ...
    private static bool FileIsOwnedByNamespacePrefix(
        ...
        IReadOnlyList<string> namespacePrefixes)
    {
        var matchingPrefixes = namespacePrefixes
            .Where(prefix => FileMatchesNamespacePrefix(fileName, prefix))
            .ToArray();
        ...
        var longestLength = matchingPrefixes.Max(static prefix => prefix.Length);
        var longestPrefixes = matchingPrefixes
            .Where(prefix => prefix.Length == longestLength)
            .ToArray();
        return longestPrefixes.Length == 1
               && longestPrefixes[0].Equals(namespacePrefix, StringComparison.OrdinalIgnoreCase);
    }
        ...
        string? containmentRoot = null)
    {
        try
        {
            ...
            for (var i = 0; i < 5 && !reader.EndOfStream; i++)
            {
                ...
                if (line != null && GeneratorUtils.ContainsAutoGeneratedMarker(line))
                {
                    return true;
                }
            }
