...
public class CodeGeneratorOrchestrator
{
    ...
    private const string ExternalOwnershipIdentityPrefix = "# Tool identity: ";
        ...
        string toolName)
    {
        ...
        if (expected.Exists)
        {
            if (!string.Equals(expected.ToolName, toolName, StringComparison.Ordinal))
            {
                throw new InvalidDataException(
                    $"External ownership manifest '{expectedRelativePath}' does not belong to tool '{toolName}'.");
            }
                     ...
                     SearchOption.TopDirectoryOnly))
        {
            ...
            if (string.Equals(candidate.ToolName, toolName, StringComparison.Ordinal))
            {
                return candidate;
            }
        }
        ...
        string relativePath)
    {
        ...
        var ownedPaths = lines
            .Where(line => !string.IsNullOrWhiteSpace(line)
                           && !line.StartsWith('#'))
            .ToArray();
        return new ExternalOwnershipManifest(relativePath, toolName, ownedPaths, Exists: true);
    }
            ...
            new[]
                {
                    ...
                    ExternalOwnershipIdentityPrefix + Convert.ToBase64String(
                        System.Text.Encoding.UTF8.GetBytes(toolName)),
                }
                .Concat(ownedPaths)
                .Append(string.Empty));
