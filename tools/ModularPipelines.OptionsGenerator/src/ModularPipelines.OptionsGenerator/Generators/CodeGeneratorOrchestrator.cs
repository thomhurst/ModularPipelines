        ...
        CancellationToken cancellationToken = default)
    {
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (File.Exists(previousManifestPath)
                && FileContainsMarker(previousManifestPath, ExternalOwnershipMarker))
            {
                DeleteFileAndRecord(outputDirectory, previousManifestPath, result.FilesDeleted);
            }
        }
        ...
        ICollection<string> deletedPaths)
    {
        ...
        foreach (var relativePath in previouslyOwnedPaths.Where(path => !current.Contains(path)))
        {
            var fullPath = ExternalToolDefinitionLoader.ValidateRelativeOutputPath(
                ...
                outputDirectory,
                "external ownership manifest path");
            if (File.Exists(fullPath) && IsExternallyOwnedGeneratedFile(fullPath))
            {
                DeleteFileAndRecord(outputDirectory, fullPath, deletedPaths);
            }
        }
    }

    private static bool IsExternallyOwnedGeneratedFile(string path) =>
        IsAutoGeneratedFile(path)
        || FileContainsMarker(
            path,
            "<!-- This file is generated by ModularPipelines.OptionsGenerator. -->")
        || CommandCoverageGuard.IsGeneratedManifest(path);
