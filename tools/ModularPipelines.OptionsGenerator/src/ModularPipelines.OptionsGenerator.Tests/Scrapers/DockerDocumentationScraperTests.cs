...
public class DockerDocumentationScraperTests
{
    ...
    [Arguments(2)]
    [Arguments(21)]
    public async Task Option_Table_Preserves_Choices_Inside_And_Outside_Enum_Limit(int count)
    {
        var values = Enumerable.Range(1, count).Select(index => $"mode{index}").ToArray();
        var hint = string.Join('|', values);
        var html = $"""
            <html><body><h1>docker build</h1>
            <table><thead><tr><th>Option</th><th>Description</th><th>Default</th></tr></thead>
            <tbody><tr><td>--mode</td><td>Select mode.</td><td>{hint}</td></tr></tbody></table>
            </body></html>
            """;
