...
public class ExternalToolDefinitionTests
{
    ...
    [Test]
    public async Task External_Metadata_Applies_Exact_Casing_To_Ownership_Manifest()
    {
        var workspace = CreateTemporaryDirectory();
        var metadataPath = Path.Combine(workspace, "private-widget.json");
        var outputDirectory = Path.Combine(workspace, "integration");
    ...
    [Test]
    public async Task External_Metadata_Preserves_Foreign_Generated_Casing_Variant()
    {
        var workspace = CreateTemporaryDirectory();
        var metadataPath = Path.Combine(workspace, "private-widget.json");
        var outputDirectory = Path.Combine(workspace, "integration");
        ...
        try
        {
            ...
            var optionsDirectory = Path.Combine(outputDirectory, "generated", "Options");
            var ownedPath = Path.Combine(optionsDirectory, "PrivateWidgetDeployOptions.Generated.cs");
            var foreignPath = Path.Combine(optionsDirectory, "PrivateWidgetDeployoptions.Generated.cs");
            const string foreignContent = "// <auto-generated>\npublic sealed class ForeignOptions;";
            ...
            await Assert.That(await File.ReadAllTextAsync(foreignPath)).IsEqualTo(foreignContent);
    ...
    [Test]
    public async Task External_Metadata_Validates_All_Casing_Variants_Before_Writing()
    {
        var workspace = CreateTemporaryDirectory();
        var metadataPath = Path.Combine(workspace, "private-widget.json");
        var outputDirectory = Path.Combine(workspace, "integration");
        ...
        try
        {
            ...
            var optionsPath = Path.Combine(
                ...
                "Options",
                "PrivateWidgetDeployOptions.Generated.cs");
            const string unchangedContent = "// existing owned output";
            await File.WriteAllTextAsync(optionsPath, unchangedContent);

            var servicesDirectory = Path.Combine(outputDirectory, "generated", "Services");
            var ownedServicePath = Path.Combine(servicesDirectory, "IPrivateWidget.Generated.cs");
            var foreignServicePath = Path.Combine(servicesDirectory, "IPrivatewidget.Generated.cs");
            const string foreignContent = "// <auto-generated>\npublic interface IForeignService;";
            File.Delete(ownedServicePath);
            await File.WriteAllTextAsync(foreignServicePath, foreignContent);

            await Assert.That(async () =>
                    await orchestrator.GenerateFromDefinitionAsync(tool, outputDirectory))
                .Throws<InvalidDataException>();
            await Assert.That(await File.ReadAllTextAsync(optionsPath)).IsEqualTo(unchangedContent);
            await Assert.That(await File.ReadAllTextAsync(foreignServicePath)).IsEqualTo(foreignContent);
            await Assert.That(File.Exists(ownedServicePath)).IsFalse();
        }
        finally
        {
            Directory.Delete(workspace, recursive: true);
    ...
    [Test]
    public async Task External_Metadata_Reconciles_Every_Recorded_Casing_Variant()
    {
        var workspace = CreateTemporaryDirectory();
        var outputDirectory = Path.Combine(workspace, "integration");
        var orchestrator = CreateOrchestrator();
        ...
        try
        {
            ...
            {
                await File.WriteAllTextAsync(
                    Path.Combine(optionsDirectory, staleName),
                    "// <auto-generated>\npublic sealed class StaleOptions;");
            }

            // An earlier generation recorded both stale variants as owned output.
            await File.AppendAllLinesAsync(
                Path.Combine(outputDirectory, ".modular-pipelines-options", $"{tool.NamespacePrefix}.files"),
                staleNames.Select(staleName => $"generated/Options/{staleName}"));
            ...
            using (Assert.Multiple())
            {
                ...
                foreach (var staleName in staleNames)
                {
                    await Assert.That(File.Exists(Path.Combine(optionsDirectory, staleName))).IsFalse();
                }
            }
        }
        finally
    ...
    private static string ValidMetadata(
        string outputDirectory,
        int schemaVersion = ExternalToolDefinitionDocument.CurrentSchemaVersion) =>
        $$"""
          {
