...
public class CodeGeneratorOrchestratorTests
{
    ...
    public async Task Duplicate_Output_Paths_Fail_Without_Mutating_Existing_Output()
    {
        ...
        {
            var scraper = new FakeCliScraper { Commands = [FakeCommand()] };
            static GeneratedFile File1() => new()
            {
    ...
    public async Task Successful_Generation_Removes_Casing_Variant_Output()
    {
        ...
        string[] staleNames = ["FakeRunoptions.Generated.cs", "FAKERUNOPTIONS.Generated.cs"];
        foreach (var staleName in staleNames)
        {
            // Both variants coexist only on case-sensitive filesystems; elsewhere they collapse to one.
            await File.WriteAllTextAsync(
                Path.Combine(optionsDirectory, staleName),
                "// <auto-generated>\npublic record FakeRunoptions;");
        }

        var oldAssemblyInfoPath = Path.Combine(outputRoot, ToolOutputDirectory, "Assemblyinfo.Generated.cs");
        await File.WriteAllTextAsync(oldAssemblyInfoPath, "// <auto-generated>");
        var originalPaths = Directory.GetFiles(optionsDirectory)
            .Append(oldAssemblyInfoPath)
        ...
        try
        {
            ...
            var result = await Orchestrator(scraper, generator).GenerateAsync("fake", outputRoot);
            var generatedFileNames = Directory.GetFiles(optionsDirectory)
                .Select(Path.GetFileName)
                .OfType<string>()
                .ToArray();

            await Assert.That(result.Errors).IsEmpty();
            await Assert.That(generatedFileNames.Count(name =>
                    name.Equals("FakeRunOptions.Generated.cs", StringComparison.OrdinalIgnoreCase)))
                .IsEqualTo(1);
            await Assert.That(generatedFileNames).Contains("FakeRunOptions.Generated.cs");
            await Assert.That(generatedFileNames).DoesNotContain("FakeRunoptions.Generated.cs");
            foreach (var originalPath in originalPaths)
            {
                await Assert.That(result.FilesDeleted).Contains(originalPath);
                await Assert.That(result.ChangedPaths).Contains(originalPath.Replace('\\', '/'));
            }

            await Assert.That(result.ChangedPaths)
                .Contains($"{ToolOutputDirectory}/Options/FakeRunOptions.Generated.cs");
        }
        finally
        {
            Directory.Delete(outputRoot, recursive: true);
    ...
    [Test]
    public async Task Generation_Preserves_Unowned_Casing_Variant_Output()
    {
        var (outputRoot, existingFile) = await CreateOutputRootWithExistingFileAsync();
        var optionsDirectory = Path.Combine(outputRoot, ToolOutputDirectory, "Options");
        File.Delete(existingFile);
        var unownedPath = Path.Combine(optionsDirectory, "FakeRunoptions.Generated.cs");
        var targetPath = Path.Combine(optionsDirectory, "FakeRunOptions.Generated.cs");
        const string unownedContent = "public record FakeRunoptions;";
        await File.WriteAllTextAsync(unownedPath, unownedContent);
        try
        {
            var scraper = new FakeCliScraper { Commands = [FakeCommand()] };
            var generator = new FakeGenerator
            ...
            using (Assert.Multiple())
            {
                ...
                await Assert.That(result.Errors[0].Message).Contains("not owned");
                await Assert.That(File.Exists(unownedPath)).IsTrue();
                await Assert.That(await File.ReadAllTextAsync(unownedPath)).IsEqualTo(unownedContent);
                await Assert.That(Directory.GetFiles(optionsDirectory)
                        .Select(Path.GetFileName)
                        .OfType<string>())
                    .DoesNotContain(Path.GetFileName(targetPath));
            }
        }
        finally
        {
