...
4. Run focused tests, broader tests, and `npm run build --prefix docs` for docs changes. Every local `dotnet` command must run through `scripts/Invoke-AgentDotNet.ps1`, invoked in-process (`& scripts/Invoke-AgentDotNet.ps1 -DotNetArguments @(...)`, or `pwsh -Command "& ..."` from a non-PowerShell shell; `pwsh scripts/Invoke-AgentDotNet.ps1 ...` flattens the argument array) with the shell/tool timeout at least 30 seconds longer than the guard timeout; never invoke `dotnet` directly. Use ModularPipelines' TUnit arguments from `CLAUDE.md`: run the relevant `*UnitTests` project with guarded arguments equivalent to `dotnet run --project <path-to-test-project> --framework net10.0`, and use `--treenode-filter` for focus. For TUnit focused runs, use one `--treenode-filter` pattern per command unless using valid TUnit OR parentheses. Guard exit 124 (timeout) or 137 (memory) is a local validation limit: do not raise the limit and retry automatically; report it in the PR and let CI run the expensive check.
...
- Run `dotnet format` through `scripts/Invoke-AgentDotNet.ps1` before committing; CI verifies formatting with the guarded equivalent of `dotnet format --verify-no-changes --severity info`.
- Never run the full build pipeline (`dotnet run` in `src/ModularPipelines.Build`) as an agent. It builds every solution and can exhaust workstation memory; CI owns full build + test + coverage validation. If Docker is unavailable for focused tests that need it, state that clearly in the PR test plan.
