...
public class GcloudRegenerationTests
{
    ...
    [Arguments("MysqlConnectionPropertiesSslKeyFile")]
    [Arguments("PostgresqlConnectionPropertiesSslKeyFile")]
    [Arguments("JavaMessageServiceConnectionPropertiesKeyStoreFile")]
    [Arguments("KafkaConnectionPropertiesKeyStoreFile")]
    [Arguments("KafkaSchemaRegistryConnectionPropertiesKeyStoreFile")]
    [Arguments("RedisConnectionPropertiesKeyStoreFile")]
    [Arguments("OracleConnectionPropertiesWalletFile")]
    ...
    {
        GcloudNotebooksInstancesCreateOptions options = selection switch
        {
            "environment" => new("instance") { Environment = "environment", EnvironmentLocation = "us-central1" },
            "location-only" => new("instance") { EnvironmentLocation = "us-central1" },
            "container" => new("instance") { ContainerRepository = "image", ContainerTag = "tag" },
            "tag-only" => new("instance") { ContainerTag = "tag" },
            "family" => new("instance") { VmImageProject = "project", VmImageFamily = "family" },
            "name" => new("instance") { VmImageProject = "project", VmImageName = "name" },
            "both-images" => new("instance") { VmImageProject = "project", VmImageFamily = "family", VmImageName = "name" },
            "mixed" => new("instance") { Environment = "environment", ContainerRepository = "image" },
    ...
    [Arguments(true)]
    [Arguments(false)]
    public async Task Bitbucket_Cloud_Requires_Its_Credentials_Alongside_The_Workspace(bool complete)
    {
        var options = new GcloudDeveloperConnectConnectionsCreateOptions("connection")
        {
            BitbucketCloudConfigWorkspace = "workspace",
            BitbucketCloudConfigAuthorizerCredentialUserTokenSecretVersion = complete ? "authorizer" : null,
            BitbucketCloudConfigReadAuthorizerCredentialUserTokenSecretVersion = complete ? "reader" : null,
            BitbucketCloudConfigWebhookSecretVersion = complete ? "webhook" : null,
        };
        var errors = new List<ValidationResult>();

        await Assert.That(Validator.TryValidateObject(options, new(options), errors, true)).IsEqualTo(complete);
    }

    [Test]
    [Arguments(true)]
    [Arguments(false)]
    public async Task Gitlab_Cloud_Requires_All_Three_Credentials(bool complete)
    {
        var options = new GcloudDeveloperConnectConnectionsCreateOptions("connection")
        {
            GitlabConfigAuthorizerCredentialUserTokenSecretVersion = "authorizer",
            GitlabConfigReadAuthorizerCredentialUserTokenSecretVersion = complete ? "reader" : null,
            GitlabConfigWebhookSecretVersion = complete ? "webhook" : null,
        };
        var errors = new List<ValidationResult>();
    ...
    [Arguments(true)]
    [Arguments(false)]
    public async Task Github_Enterprise_Requires_Its_Host_When_Organization_Is_Set(bool includeHost)
    {
        var options = new GcloudDeveloperConnectConnectionsCreateOptions("connection")
        {
            GithubEnterpriseConfigHostUri = includeHost ? "https://example.com" : null,
            GithubEnterpriseConfigOrganization = "organization",
        };
        var errors = new List<ValidationResult>();

        await Assert.That(Validator.TryValidateObject(options, new(options), errors, true)).IsEqualTo(includeHost);
    }
    ...
    [Test]
    public async Task Multiword_Credential_Resource_Identifier_Is_Visible()
    {
        var property = typeof(GcloudIamOauthClientsCredentialsDescribeOptions)
            .GetProperty(nameof(GcloudIamOauthClientsCredentialsDescribeOptions.Credential))!;
        await Assert.That(property.GetCustomAttribute<SecretValueAttribute>()).IsNull();
        await AssertArguments(BuildArguments(new GcloudIamOauthClientsCredentialsDescribeOptions("credential-id")
        {
            Location = "global",
            OauthClient = "client-id",
        }), ["credential-id", "--location=global", "--oauth-client=client-id"]);
    }

    [Test]
    public async Task Multiword_Scim_Token_Resource_Identifier_Is_Visible()
    {
        var property = typeof(GcloudIamWorkforcePoolsProvidersScimTenantsTokensDescribeOptions)
            .GetProperty(nameof(GcloudIamWorkforcePoolsProvidersScimTenantsTokensDescribeOptions.Token))!;
        await Assert.That(property.GetCustomAttribute<SecretValueAttribute>()).IsNull();
        await AssertArguments(BuildArguments(new GcloudIamWorkforcePoolsProvidersScimTenantsTokensDescribeOptions("token-id")
        {
            Location = "global",
            Provider = "provider-id",
            ScimTenant = "tenant-id",
            WorkforcePool = "pool-id",
        }), ["token-id", "--location=global", "--provider=provider-id", "--scim-tenant=tenant-id", "--workforce-pool=pool-id"]);
    }
