...
- **Sub-Modules**: Both `context.SubModule<T>(name, action)` (returns value) and `context.SubModule(name, action)` (no return) are fully supported. Use sub-modules to track iterations within a module (e.g., packing multiple NuGet packages). See [Sub-Modules documentation](/docs/how-to/sub-modules).
