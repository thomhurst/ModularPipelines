...
Resolve the service with `context.Tools.Liquibase`. The `context.Liquibase()` extension method remains available as a compatibility fallback for projects older than C# 14.
