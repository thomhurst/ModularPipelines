...
Resolve the service with `context.Tools.Pip`. The `context.Pip()` extension method remains available as a compatibility fallback for projects older than C# 14.
