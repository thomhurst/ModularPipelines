// <auto-generated>
// This file was generated by ModularPipelines.OptionsGenerator.
// Do not edit this file manually.
// </auto-generated>

#nullable enable

using System.CodeDom.Compiler;
using System.Diagnostics.CodeAnalysis;
using ModularPipelines.Attributes;
using ModularPipelines.Homebrew.Options;

namespace ModularPipelines.Homebrew.Options;

/// <summary>
/// Displays out-of-date packages and the latest version available. If the returned
/// </summary>
[GeneratedCode("ModularPipelines.OptionsGenerator", "2.0.0")]
[ExcludeFromCodeCoverage]
[CliSubCommand("bump")]
public record BrewBumpOptions : BrewOptions
{
    /// <summary>
    /// Print formulae/casks with fully-qualified
    /// </summary>
    [CliFlag("--full-name")]
    public bool? FullName { get; set; }

    /// <summary>
    /// Do not retrieve pull requests from GitHub.
    /// </summary>
    [CliFlag("--no-pull-requests")]
    public bool? NoPullRequests { get; set; }

    /// <summary>
    /// Ignore formulae/casks in autobump list
    /// </summary>
    [CliFlag("--no-autobump")]
    public bool? NoAutobump { get; set; }

    /// <summary>
    /// Check only formulae.
    /// </summary>
    [CliFlag("--formula")]
    public bool? Formula { get; set; }

    /// <summary>
    /// Check only casks.
    /// </summary>
    [CliFlag("--cask")]
    public bool? Cask { get; set; }

    /// <summary>
    /// Use Repology to check for outdated packages.
    /// </summary>
    [CliFlag("--repology")]
    public bool? Repology { get; set; }

    /// <summary>
    /// Check formulae and casks within the given
    /// </summary>
    [CliFlag("--tap")]
    public bool? Tap { get; set; }

    /// <summary>
    /// Check formulae and casks that are currently
    /// </summary>
    [CliFlag("--installed")]
    public bool? Installed { get; set; }

    /// <summary>
    /// Don't try to fork the repository.
    /// </summary>
    [CliFlag("--no-fork")]
    public bool? NoFork { get; set; }

    /// <summary>
    /// Open a pull request for the new version if
    /// </summary>
    [CliFlag("--open-pr")]
    public bool? OpenPr { get; set; }

    /// <summary>
    /// Letter or word that the list of package
    /// </summary>
    [CliFlag("--start-with")]
    public bool? StartWith { get; set; }

    /// <summary>
    /// Bump additional formulae marked as synced
    /// </summary>
    [CliFlag("--bump-synced")]
    public bool? BumpSynced { get; set; }

    /// <summary>
    /// Display any debugging information.
    /// </summary>
    [CliFlag("--debug", ShortForm = "-d")]
    public bool? Debug { get; set; }

    /// <summary>
    /// Make some output more quiet.
    /// </summary>
    [CliFlag("--quiet", ShortForm = "-q")]
    public bool? Quiet { get; set; }

    /// <summary>
    /// Make some output more verbose.
    /// </summary>
    [CliFlag("--verbose", ShortForm = "-v")]
    public bool? Verbose { get; set; }

    /// <summary>
    /// Show this message.
    /// </summary>
    [CliFlag("--help", ShortForm = "-h")]
    public bool? Help { get; set; }

    /// <summary>
    /// The formula operand.
    /// </summary>
    [CliArgument(0, Phase = CommandLinePhase.Passthrough)]
    public IEnumerable<string>? FormulaOperand { get; set; }

    [Obsolete("EvalAll is no longer supported by the installed CLI and has no effect.")]
    public bool? EvalAll { get; set; }

}
