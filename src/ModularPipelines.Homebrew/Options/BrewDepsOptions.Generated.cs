// <auto-generated>
// This file was generated by ModularPipelines.OptionsGenerator.
// Do not edit this file manually.
// </auto-generated>

#nullable enable

using System.CodeDom.Compiler;
using System.Diagnostics.CodeAnalysis;
using ModularPipelines.Attributes;
using ModularPipelines.Homebrew.Options;

namespace ModularPipelines.Homebrew.Options;

/// <summary>
/// Show dependencies for formula. When given multiple formula arguments, show the
/// </summary>
[GeneratedCode("ModularPipelines.OptionsGenerator", "2.0.0")]
[ExcludeFromCodeCoverage]
[CliSubCommand("deps")]
public record BrewDepsOptions : BrewOptions
{
    /// <summary>
    /// Sort dependencies in topological order.
    /// </summary>
    [CliFlag("--topological", ShortForm = "-n")]
    public bool? Topological { get; set; }

    /// <summary>
    /// Show only the direct dependencies declared in
    /// </summary>
    [CliFlag("--direct", ShortForm = "-1")]
    public bool? Direct { get; set; }

    /// <summary>
    /// Show the union of dependencies for multiple
    /// </summary>
    [CliFlag("--union")]
    public bool? Union { get; set; }

    /// <summary>
    /// List dependencies by their full name.
    /// </summary>
    [CliFlag("--full-name")]
    public bool? FullName { get; set; }

    /// <summary>
    /// Include implicit dependencies used to
    /// </summary>
    [CliFlag("--include-implicit")]
    public bool? IncludeImplicit { get; set; }

    /// <summary>
    /// Include :build dependencies for formula.
    /// </summary>
    [CliFlag("--include-build")]
    public bool? IncludeBuild { get; set; }

    /// <summary>
    /// Include :optional dependencies for
    /// </summary>
    [CliFlag("--include-optional")]
    public bool? IncludeOptional { get; set; }

    /// <summary>
    /// Include :test dependencies for formula
    /// </summary>
    [CliFlag("--include-test")]
    public bool? IncludeTest { get; set; }

    /// <summary>
    /// Skip :recommended dependencies for
    /// </summary>
    [CliFlag("--skip-recommended")]
    public bool? SkipRecommended { get; set; }

    /// <summary>
    /// Include requirements in addition to
    /// </summary>
    [CliFlag("--include-requirements")]
    public bool? IncludeRequirements { get; set; }

    /// <summary>
    /// Show dependencies as a tree. When given
    /// </summary>
    [CliFlag("--tree")]
    public bool? Tree { get; set; }

    /// <summary>
    /// Prune parts of tree already seen.
    /// </summary>
    [CliFlag("--prune")]
    public bool? Prune { get; set; }

    /// <summary>
    /// Show dependencies as a directed graph.
    /// </summary>
    [CliFlag("--graph")]
    public bool? Graph { get; set; }

    /// <summary>
    /// Show text-based graph description in DOT
    /// </summary>
    [CliFlag("--dot")]
    public bool? Dot { get; set; }

    /// <summary>
    /// Mark any build, test, implicit, optional, or
    /// </summary>
    [CliFlag("--annotate")]
    public bool? Annotate { get; set; }

    /// <summary>
    /// List dependencies for formulae that are
    /// </summary>
    [CliFlag("--installed")]
    public bool? Installed { get; set; }

    /// <summary>
    /// Use formulae and casks listed in a Brewfile
    /// </summary>
    [CliOption("--brewfile", Format = OptionFormat.EqualsSeparated)]
    public string? Brewfile { get; set; }

    /// <summary>
    /// Show only missing dependencies.
    /// </summary>
    [CliFlag("--missing")]
    public bool? Missing { get; set; }

    /// <summary>
    /// Switch into the mode used when evaluating all
    /// </summary>
    [CliFlag("--for-each")]
    public bool? ForEach { get; set; }

    /// <summary>
    /// Show dependencies for HEAD version instead of
    /// </summary>
    [CliFlag("--HEAD")]
    public bool? Head { get; set; }

    /// <summary>
    /// Show dependencies for the given operating
    /// </summary>
    [CliFlag("--os")]
    public bool? Os { get; set; }

    /// <summary>
    /// Show dependencies for the given CPU
    /// </summary>
    [CliFlag("--arch")]
    public bool? Arch { get; set; }

    /// <summary>
    /// Treat all named arguments as formulae.
    /// </summary>
    [CliFlag("--formula")]
    public bool? Formula { get; set; }

    /// <summary>
    /// Treat all named arguments as casks.
    /// </summary>
    [CliFlag("--cask")]
    public bool? Cask { get; set; }

    /// <summary>
    /// Display any debugging information.
    /// </summary>
    [CliFlag("--debug", ShortForm = "-d")]
    public bool? Debug { get; set; }

    /// <summary>
    /// Make some output more quiet.
    /// </summary>
    [CliFlag("--quiet", ShortForm = "-q")]
    public bool? Quiet { get; set; }

    /// <summary>
    /// Make some output more verbose.
    /// </summary>
    [CliFlag("--verbose", ShortForm = "-v")]
    public bool? Verbose { get; set; }

    /// <summary>
    /// Show this message.
    /// </summary>
    [CliFlag("--help", ShortForm = "-h")]
    public bool? Help { get; set; }

    /// <summary>
    /// The formula operand.
    /// </summary>
    [CliArgument(0, Phase = CommandLinePhase.Passthrough)]
    public IEnumerable<string>? FormulaOperand { get; set; }

    [Obsolete("EvalAll is no longer supported by the installed CLI and has no effect.")]
    public bool? EvalAll { get; set; }

}
