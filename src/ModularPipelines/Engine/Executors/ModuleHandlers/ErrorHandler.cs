...
internal class ErrorHandler<T> : BaseHandler<T>, IErrorHandler<T>
{
    ...
    {
        var logger = Module.Context.Get<ILogger<ErrorHandler<T>>>()!;

        logger.LogDebug("{Module} has thrown {Exception}", Module.GetType().Name, exception.GetType().Name);
        
        Context.Logger.LogError(exception, "Module Failed after {Duration}", Module.Duration);

        if (IsModuleTimedOutException(exception))
        {
            logger.LogDebug("{Module} has timed out", Module.GetType().Name);
        ...
        {
            logger.LogDebug("{Module} has terminated because of pipeline cancellation", Module.GetType().Name);
        ...
        {
            logger.LogDebug("{Module} is ignoring its exception and continuing the pipeline", Module.GetType().Name);
    ...
    {
        Module.Context.Get<ILogger<ErrorHandler<T>>>()!.LogDebug("{Module} is throwing its exception", Module.GetType().Name);
