...
public record GcloudDataplexDatascansCreateDataQualityOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// create a Dataplex data     quality scan job
    /// </summary>
    ...
    /// <param name="Datascan">Datascan resource - Arguments and flags that define the Dataplex datascan you want to create a data quality scan for. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways. To set the project attribute: ◆ provide the argument datascan on the command line with a fully specified name; ◆ provide the argument --project on the command line; ◆ set the property core/project. This must be specified. ID of the datascan or fully qualified identifier for the datascan. To set the dataScans attribute: ▸ provide the argument datascan on the command line. This positional argument must be specified if any of the other arguments in this group are specified.</param>
    public GcloudDataplexDatascansCreateDataQualityOptions(
        string DataQualitySpecFile,
        string Datascan
    )
    {
        global::System.ArgumentNullException.ThrowIfNull(DataQualitySpecFile);
        this.DataQualitySpecFile = DataQualitySpecFile;
        global::System.ArgumentNullException.ThrowIfNull(Datascan);
        this.Datascan = Datascan;
    }

    public void Deconstruct(out string DataQualitySpecFile, out string Datascan)
    {
        DataQualitySpecFile = this.DataQualitySpecFile;
    ...
    /// <summary>
    /// Data quality scan scheduling and trigger settings At most one of these can be specified: If set, the scan runs one-time shortly after data quality scan creation.
    /// </summary>
    [CliOption("--on-demand", Format = OptionFormat.EqualsSeparated)]
    public string? OnDemand { get; set; }

    /// <summary>
    /// Data quality scan scheduling and trigger settings At most one of these can be specified: Cron schedule (https://en.wikipedia.org/wiki/Cron) for running scans periodically. To explicitly set a timezone to the cron tab, apply a prefix in the cron tab: "CRON_TZ=${IANA_TIME_ZONE}" or "TZ=${IANA_TIME_ZONE}". The ${IANA_TIME_ZONE} may only be a valid string from IANA time zone database. For example, CRON_TZ=America/New_York 1 * * * * or TZ=America/New_York 1 * * * *. This field is required for RECURRING scans.
    /// </summary>
    [CliOption("--schedule", Format = OptionFormat.EqualsSeparated)]
    public string? Schedule { get; set; }

    /// <summary>
    /// Data quality scan scheduling and trigger settings At most one of these can be specified: Or at least one of these can be specified: Data quality scan one-time trigger settings. If set, the data quality scan runs once, and auto deleted once the ttl_after_scan_completion expires.
    /// </summary>
    [CliFlag("--one-time")]
    public bool? OneTime { get; set; }

    /// <summary>
    /// Data quality scan scheduling and trigger settings At most one of these can be specified: Or at least one of these can be specified: Data quality scan one-time trigger settings. The time to live for one-time scans. Default value is 24 hours, minimum value is 0 seconds, and maximum value is 365 days. The time is calculated from the data scan job completion time. If value is set as 0 seconds, the scan will be immediately deleted upon job completion, regardless of whether the job succeeded or failed. The value should be a number followed by a unit suffix "s". Example: "100s" for 100 seconds.The argument is only valid when --one-time is set.
    /// </summary>
