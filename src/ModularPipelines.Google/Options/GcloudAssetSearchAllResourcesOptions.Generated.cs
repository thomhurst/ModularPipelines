...
public record GcloudAssetSearchAllResourcesOptions : GcloudOptions
{
    ...
    /// <summary>
    /// A comma-separated list of fields specifying which fields to be returned in the results. Only "*" or combination of top level fields can be specified. Examples: "*", "name,location", "name,versionedResources". The read_mask paths must be valid field paths listed but not limited to the following (both snake_case and camelCase are supported): ◆ name ◆ asset_type or assetType ◆ project ◆ display_name or displayName ◆ description ◆ location ◆ labels ◆ tags ◆ effective_tags or effectiveTags ◆ network_tags or networkTags ◆ kms_keys or kmsKeys ◆ create_time or createTime ◆ update_time or updateTime ◆ state ◆ additional_attributes or additionalAttributes ◆ versioned_resources or versionedResources If read_mask is not specified, all fields except versionedResources will be returned. If only "*" is specified, all fields including versionedResources will be returned. Collection entries are joined with commas into one option value. For entries containing commas, supply one pre-escaped list value using gcloud topic escaping (https://cloud.google.com/sdk/gcloud/reference/topic/escaping).
    /// </summary>
    [CliOption("--read-mask", Format = OptionFormat.EqualsSeparated, CollectionSeparator = ",")]
    public IEnumerable<string>? ReadMask { get; set; }
