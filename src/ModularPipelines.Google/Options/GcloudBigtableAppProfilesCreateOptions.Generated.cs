...
{
    /// <summary>
    /// Exactly one of these must be specified: Multi Cluster Routing Policy Single Cluster Routing Policy Use Multi Cluster Routing policy. This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    [CliFlag("--route-any")]
    public bool? RouteAny { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Multi Cluster Routing Policy Single Cluster Routing Policy Cluster IDs to route to using the Multi Cluster Routing Policy. If unset, all clusters in the instance are eligible.
    /// </summary>
    [CliOption("--restrict-to", Format = OptionFormat.EqualsSeparated)]
    public IEnumerable<string>? RestrictTo { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Multi Cluster Routing Policy Single Cluster Routing Policy Use row-affinity routing for this app profile.
    /// </summary>
    [CliFlag("--row-affinity")]
    public bool? RowAffinity { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Multi Cluster Routing Policy Single Cluster Routing Policy Cluster ID to route to using Single Cluster Routing policy. This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    [CliOption("--route-to", Format = OptionFormat.EqualsSeparated)]
    public string? RouteTo { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Multi Cluster Routing Policy Single Cluster Routing Policy Allow transactional writes with a Single Cluster Routing policy.
    /// </summary>
    ...
    {
        if ((RouteAny == true ? 1 : 0) + (RestrictTo?.Any() == true ? 1 : 0) + (RowAffinity == true ? 1 : 0) + (!string.IsNullOrWhiteSpace(RouteTo) ? 1 : 0) + (TransactionalWrites == true ? 1 : 0) != 1)
        {
            yield return new ValidationResult("Exactly one of RouteAny, RestrictTo, RowAffinity, RouteTo, or TransactionalWrites must be specified.", [nameof(RouteAny), nameof(RestrictTo), nameof(RowAffinity), nameof(RouteTo), nameof(TransactionalWrites)]);
        }
    }
