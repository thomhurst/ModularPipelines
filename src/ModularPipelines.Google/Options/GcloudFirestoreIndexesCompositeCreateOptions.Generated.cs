...
public record GcloudFirestoreIndexesCompositeCreateOptions : GcloudOptions
{
    ...
    {
        {
            ...
            FieldConfig = materialized;
        }
        this.FieldConfig = FieldConfig;
        global::System.ArgumentNullException.ThrowIfNull(CollectionGroup);
        this.CollectionGroup = CollectionGroup;
    }

    public void Deconstruct(out IEnumerable<string> FieldConfig, out string CollectionGroup)
    {
    ...
    /// <summary>
    /// Required, Configuration for an index field. array-config Specifies the configuration for an array field. The only valid option is 'contains'. Exactly one of 'order', 'array-config', or 'vector-config' must be specified. field-path Required, specifies the field path (e.g. address.city). order Specifies the order. Valid options are 'ascending', 'descending'. Exactly one of 'order', 'array-config', or 'vector-config' must be specified. search-config Specifies the configuration for a search field. An index definition must contain either only 'search-config' fields or only non 'search-config' fields. The following shorthand aliases are supported instead of a full 'search-config': ▸ TEXT_TOKENIZED_MATCH_GLOBALLY: Tokenized text search with global matching. ▸ GEO_POINT: Geo search. Examples: With alias: --field-config=field-path=title,search-config=TEXT_TOKENIZED_MATCH_GLOBALLY Text search: --field-config=field-path=title,search-config='{"text-spec": {"index-specs": [{"index-type": "tokenized", "match-type": "match-globally"}]}}' Geo search: --field-config=field-path=location,search-config='{"geo-spec": {"geo-json-indexing-disabled": true}}' With file: --field-config=field-path=text,search-config='/path/to/configs/search-config.json' For complex configurations, it is recommended to use a file. geo-spec Optional. The specification for building a geo search index for a field. geo-json-indexing-disabled Optional. Disables geoJSON indexing for the field. By default, geoJSON points are indexed. text-spec Optional. The specification for building a text search index for a field. index-specs Optional. Array of specifications for how the field should be indexed. index-type Required. How to index the text field value. match-type Required. How to match the text field value. vector-config Specifies the configuration for a vector field. Exactly one of 'order', 'array-config', or 'vector-config' must be specified. dimension Required, sets dimension value. flat Sets flat value. Shorthand Example: --field-config=array-config=string,field-path=string,order=string,search-config=geo-spec={geo-json-indexing-disabled=boolean},text-spec={index-specs=[{index-type=string,match-type=string}]},vector-config={dimension=int,flat} --field-config=array-config=string,field-path=string,order=string,search-config=geo-spec={geo-json-indexing-disabled=boolean},text-spec={index-specs=[{index-type=string,match-type=string}]},vector-config={dimension=int,flat} JSON Example: --field-config='[{"array-config": "string", "field-path": "string", "order": "string", "search-config": {"geo-spec": {"geo-json-indexing-disabled": boolean}, "text-spec": {"index-specs": [{"index-type": "string", "match-type": "string"}]}}, "vector-config": {"dimension": int, "flat": {}}}]' File Example: --field-config=path_to_file.(yaml|json)
    /// </summary>
    [CliOption("--field-config", Format = OptionFormat.EqualsSeparated)]
    public IEnumerable<string> FieldConfig { get; private init; }
    ...
    public string CollectionGroup { get; private init; }
    ...
    public GcloudFirestoreIndexesCompositeCreateApiScope? ApiScope { get; set; }
    ...
    public GcloudFirestoreIndexesCompositeCreateDensity? Density { get; set; }
    ...
    public GcloudFirestoreIndexesCompositeCreateQueryScope? QueryScope { get; set; }
