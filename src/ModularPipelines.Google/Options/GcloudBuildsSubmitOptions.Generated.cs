...
{
    /// <summary>
    /// Worker pool only flags. Return immediately, without waiting for the operation in progress to complete.
    /// </summary>
    [CliFlag("--async")]
    public bool? Async { get; set; }

    /// <summary>
    /// Worker pool only flags. If set, disable layer caching when building with Kaniko. This has the same effect as setting the builds/kaniko_cache_ttl property to 0 for this build. This can be useful in cases where Dockerfile builds are non-deterministic and a non-deterministic result should not be cached.
    /// </summary>
    [CliFlag("--no-cache")]
    public bool? NoCache { get; set; }

    /// <summary>
    /// Worker pool only flags. How default buckets are setup. DEFAULT_BUCKETS_BEHAVIOR must be one of: default-logs-bucket-behavior-unspecified, legacy-bucket, regional-user-owned-bucket.
    /// </summary>
    [CliOption("--default-buckets-behavior", Format = OptionFormat.EqualsSeparated)]
    public string? DefaultBucketsBehavior { get; set; }

    /// <summary>
    /// Worker pool only flags. Directory, relative to the source root, in which to run the build. This is used when the build source is a 2nd-gen Cloud Build repository resource, or a Developer Connect GitRepositoryLink resource. This must be a relative path. If a step's dir is specified and is an absolute path, this value is ignored for that step's execution.
    /// </summary>
    [CliOption("--dir", Format = OptionFormat.EqualsSeparated)]
    public string? Dir { get; set; }

    /// <summary>
    /// Worker pool only flags. Machine disk size (GB) to run the build.
    /// </summary>
    [CliOption("--disk-size", Format = OptionFormat.EqualsSeparated)]
    public int? DiskSize { get; set; }

    /// <summary>
    /// Worker pool only flags. A directory in Google Cloud Storage to hold build logs. If this field is not set, gs://[PROJECT_NUMBER].cloudbuild-logs.googleusercontent.com/ will be created and used or gs://[PROJECT_NUMBER]-[builds/region]-cloudbuild-logs is used when you set --default-buckets-behavior to REGIONAL_USER_OWNED_BUCKET.
    /// </summary>
    ...
    /// <summary>
    /// Worker pool only flags. Directory, relative to the source root, in which to run the build. This must be a relative path. If a step's dir is specified and is an absolute path, this value is ignored for that step's execution.
    /// </summary>
    [CliOption("--git-source-dir", Format = OptionFormat.EqualsSeparated)]
    public string? GitSourceDir { get; set; }

    /// <summary>
    /// Worker pool only flags. Revision to fetch from the Git repository such as a branch, a tag, a commit SHA, or any Git ref to run the build. Cloud Build uses git fetch to fetch the revision from the Git repository; therefore make sure that the string you provide for revision is parsable by the command. For information on string values accepted by git fetch, see https://git-scm.com/docs/gitrevisions#_specifying_revisions. For information on git fetch, see https://git-scm.com/docs/git-fetch.
    /// </summary>
    [CliOption("--git-source-revision", Format = OptionFormat.EqualsSeparated)]
    public string? GitSourceRevision { get; set; }

    /// <summary>
    /// Worker pool only flags. Override the .gcloudignore file and use the specified file instead.
    /// </summary>
    [CliOption("--ignore-file", Format = OptionFormat.EqualsSeparated)]
    public string? IgnoreFile { get; set; }

    /// <summary>
    /// Worker pool only flags. Machine type used to run the build. MACHINE_TYPE must be one of: e2-highcpu-32, e2-highcpu-8, e2-medium, e2-standard-2, n1-highcpu-32, n1-highcpu-8.
    /// </summary>
    [CliOption("--machine-type", Format = OptionFormat.EqualsSeparated)]
    public GcloudMachineType? MachineType { get; set; }

    /// <summary>
    /// Worker pool only flags. Amount of time in seconds to wait between polling build status.
    /// </summary>
    [CliOption("--polling-interval", Format = OptionFormat.EqualsSeparated)]
    public string? PollingInterval { get; set; }

    /// <summary>
    /// Worker pool only flags. The region of the Cloud Build Service to use. Must be set to a supported region name (e.g. us-central1). If unset, builds/region, which is the default region to use when working with Cloud Build resources, is used. If builds/region is unset, region is set to global. Note: Region must be specified in 2nd gen repo; global is not supported.
    /// </summary>
