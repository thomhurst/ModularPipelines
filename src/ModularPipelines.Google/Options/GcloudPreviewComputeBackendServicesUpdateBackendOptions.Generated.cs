...
// Source: https://cloud.google.com/sdk/gcloud/reference/preview/compute/backend-services/update-backend
...
[CliSubCommand("preview", "compute", "backend-services", "update-backend")]
public record GcloudPreviewComputeBackendServicesUpdateBackendOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// update an existing     backend of a load balancer or Traffic Director
    /// </summary>
    /// <param name="BackendServiceName">Name of the backend service to operate on.</param>
    public GcloudPreviewComputeBackendServicesUpdateBackendOptions(
    ...
    /// <summary>
    /// Exactly one of these must be specified: Instance Group Name of the instance group to update in the backend service. For details on valid instance names, refer to the criteria documented under the field 'name' at: https://cloud.google.com/compute/docs/reference/rest/v1/instances This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    [CliOption("--instance-group", Format = OptionFormat.EqualsSeparated)]
    public string? InstanceGroup { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Instance Group At most one of these can be specified: Region of the instance group to update in the backend service. If not specified, you might be prompted to select a region (interactive mode only). To avoid prompting when this flag is omitted, you can set the compute/region property: $ gcloud config set compute/region REGION A list of regions can be fetched by running: $ gcloud compute regions list To unset the property, run: $ gcloud config unset compute/region Alternatively, the region can be stored in the environment variable CLOUDSDK_COMPUTE_REGION.
    /// </summary>
    [CliOption("--instance-group-region", Format = OptionFormat.EqualsSeparated)]
    public string? InstanceGroupRegion { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Instance Group At most one of these can be specified: Zone of the instance group to update in the backend service. If not specified and the compute/zone property isn't set, you might be prompted to select a zone (interactive mode only). To avoid prompting when this flag is omitted, you can set the compute/zone property: $ gcloud config set compute/zone ZONE A list of zones can be fetched by running: $ gcloud compute zones list To unset the property, run: $ gcloud config unset compute/zone Alternatively, the zone can be stored in the environment variable CLOUDSDK_COMPUTE_ZONE.
    /// </summary>
    [CliOption("--instance-group-zone", Format = OptionFormat.EqualsSeparated)]
    public string? InstanceGroupZone { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Network Endpoint Group Name of the network endpoint group to update in the backend service. This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    [CliOption("--network-endpoint-group", Format = OptionFormat.EqualsSeparated)]
    public string? NetworkEndpointGroup { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: Network Endpoint Group Zone of the network endpoint group to update in the backend service. If not specified and the compute/zone property isn't set, you might be prompted to select a zone (interactive mode only). To avoid prompting when this flag is omitted, you can set the compute/zone property: $ gcloud config set compute/zone ZONE A list of zones can be fetched by running: $ gcloud compute zones list To unset the property, run: $ gcloud config unset compute/zone Alternatively, the zone can be stored in the environment variable CLOUDSDK_COMPUTE_ZONE.
    /// </summary>
    ...
    /// <summary>
    /// At most one of these can be specified: List of custom metrics that are used for CUSTOM_METRICS balancing mode and WEIGHTED_ROUND_ROBIN locality load balancing policy. Example: $ gcloud preview compute backend-services update-backend \ --custom-metrics='name=my-signal,maxUtilization=0.8,dryRun=true' $ gcloud preview compute backend-services update-backend \ --custom-metrics='name=my-signal,maxUtilization=0.8,dryRun=true'\ --custom-metrics='name=my-signal2,maxUtilization=0.2' $ gcloud preview compute backend-services update-backend \ --custom-metrics='[{"name" : "my-signal", "maxUtilization" : 0.8, "dryRun" : true}, {"name" : "my-signal2", "maxUtilization" : 0.1}]' Sets custom_metrics value. dryRun Sets dryRun value. maxUtilization Sets maxUtilization value. name Required, sets name value. Shorthand Example: --custom-metrics=dryRun=boolean,maxUtilization=float,name=string --custom-metrics=dryRun=boolean,maxUtilization=float,name=string JSON Example: --custom-metrics='[{"dryRun": boolean, "maxUtilization": float, "name": "string"}]' File Example: --custom-metrics=path_to_file.(yaml|json)
    /// </summary>
    ...
    {
        if (((!string.IsNullOrWhiteSpace(InstanceGroup) || !string.IsNullOrWhiteSpace(InstanceGroupRegion) || !string.IsNullOrWhiteSpace(InstanceGroupZone)) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(NetworkEndpointGroup) || !string.IsNullOrWhiteSpace(NetworkEndpointGroupZone)) ? 1 : 0) != 1)
        {
            yield return new ValidationResult("Exactly one of (InstanceGroup, InstanceGroupRegion, or InstanceGroupZone) or (NetworkEndpointGroup or NetworkEndpointGroupZone) must be specified.", [nameof(InstanceGroup), nameof(InstanceGroupRegion), nameof(InstanceGroupZone), nameof(NetworkEndpointGroup), nameof(NetworkEndpointGroupZone)]);
        }
        if ((!string.IsNullOrWhiteSpace(InstanceGroup) || !string.IsNullOrWhiteSpace(InstanceGroupRegion) || !string.IsNullOrWhiteSpace(InstanceGroupZone) || !string.IsNullOrWhiteSpace(NetworkEndpointGroup) || !string.IsNullOrWhiteSpace(NetworkEndpointGroupZone)) && (!string.IsNullOrWhiteSpace(InstanceGroup) || !string.IsNullOrWhiteSpace(InstanceGroupRegion) || !string.IsNullOrWhiteSpace(InstanceGroupZone)) && (!(!string.IsNullOrWhiteSpace(InstanceGroup))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(InstanceGroup) || !string.IsNullOrWhiteSpace(InstanceGroupRegion) || !string.IsNullOrWhiteSpace(InstanceGroupZone) || !string.IsNullOrWhiteSpace(NetworkEndpointGroup) || !string.IsNullOrWhiteSpace(NetworkEndpointGroupZone)) && (!string.IsNullOrWhiteSpace(NetworkEndpointGroup) || !string.IsNullOrWhiteSpace(NetworkEndpointGroupZone)) && (!(!string.IsNullOrWhiteSpace(NetworkEndpointGroup))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(MaxConnections) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxConnectionsPerEndpoint) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxConnectionsPerInstance) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxInFlightRequests) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxInFlightRequestsPerEndpoint) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxInFlightRequestsPerInstance) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxRate) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxRatePerEndpoint) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MaxRatePerInstance) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of MaxConnections, MaxConnectionsPerEndpoint, MaxConnectionsPerInstance, MaxInFlightRequests, MaxInFlightRequestsPerEndpoint, MaxInFlightRequestsPerInstance, MaxRate, MaxRatePerEndpoint, or MaxRatePerInstance may be specified.", [nameof(MaxConnections), nameof(MaxConnectionsPerEndpoint), nameof(MaxConnectionsPerInstance), nameof(MaxInFlightRequests), nameof(MaxInFlightRequestsPerEndpoint), nameof(MaxInFlightRequestsPerInstance), nameof(MaxRate), nameof(MaxRatePerEndpoint), nameof(MaxRatePerInstance)]);
        }
        yield break;
    }
