...
public record GcloudWorkstationsConfigsCreateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: Number of seconds to wait after initially creating or subsequently shutting down the workstation before converting its disk into a snapshot to save costs. A value of 0 indicates that the disk will never be archived.
    /// </summary>
    [CliOption("--disk-archive-timeout", Format = OptionFormat.EqualsSeparated)]
    public int? DiskArchiveTimeout { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: What should happen to the disk after the Workstation is deleted. DISK_RECLAIM_POLICY must be one of: delete The persistent disk will be deleted with the Workstation. retain The persistent disk will be remain after the workstation is deleted and the administrator must manually delete the disk.
    /// </summary>
    [CliOption("--disk-reclaim-policy", Format = OptionFormat.EqualsSeparated)]
    public string? DiskReclaimPolicy { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: Type of the persistent directory. DISK_TYPE must be one of: pd-standard, pd-balanced, pd-ssd, hyperdisk-balanced-ha.
    /// </summary>
    [CliOption("--disk-type", Format = OptionFormat.EqualsSeparated)]
    public GcloudWorkstationsConfigsCreateDiskType? DiskType { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: At most one of these can be specified: Size of the persistent directory in GB. DISK_SIZE must be one of: 10, 50, 100, 200, 500, 1000.
    /// </summary>
    [CliOption("--disk-size", Format = OptionFormat.EqualsSeparated)]
    public int? DiskSize { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: At most one of these can be specified: Name of the snapshot to use as the source for the home disk.
    /// </summary>
    [CliOption("--disk-source-snapshot", Format = OptionFormat.EqualsSeparated)]
    public string? DiskSourceSnapshot { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: Type of the persistent directory. PD_DISK_TYPE must be one of: pd-standard, pd-balanced, pd-ssd.
    /// </summary>
    [CliOption("--pd-disk-type", Format = OptionFormat.EqualsSeparated)]
    public GcloudWorkstationsConfigsCreatePdDiskType? PdDiskType { get; set; }
    ...
    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: At most one of these can be specified: Size of the persistent directory in GB. PD_DISK_SIZE must be one of: 10, 50, 100, 200, 500, 1000.
    /// </summary>
    ...
    {
        if (((object?)AcceleratorCount is not null || !string.IsNullOrWhiteSpace(AcceleratorType)) && (!((object?)AcceleratorCount is not null)))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(ContainerCustomImage) ? 1 : 0) + (!string.IsNullOrWhiteSpace(ContainerPredefinedImage) ? 1 : 0) > 1)
        {
        ...
        if ((!string.IsNullOrWhiteSpace(KmsKey) || !string.IsNullOrWhiteSpace(KmsKeyServiceAccount)) && (!(!string.IsNullOrWhiteSpace(KmsKey))))
        {
