...
public record GcloudSqlImportBakOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// Exactly one of these must be specified: Prompt for the private key password associated with the BAK file with character echo disabled. The password is all typed characters up to but not including the RETURN or ENTER key.
    /// </summary>
    [CliFlag("--prompt-for-pvk-password")]
    public bool? PromptForPvkPassword { get; set; }

    /// <summary>
    /// Exactly one of these must be specified: The private key password associated with the BAK file.
    /// </summary>
    [SecretValue]
    ...
    {
        if ((!string.IsNullOrWhiteSpace(CertPath) || !string.IsNullOrWhiteSpace(PvkPath)) && (!(!string.IsNullOrWhiteSpace(CertPath))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(CertPath) || !string.IsNullOrWhiteSpace(PvkPath)) && (!(!string.IsNullOrWhiteSpace(PvkPath))))
        {
        ...
        {
            yield return new ValidationResult("At most one of PromptForPvkPassword or PvkPassword may be specified.", [nameof(PromptForPvkPassword), nameof(PvkPassword)]);
        }
        yield break;
    }
