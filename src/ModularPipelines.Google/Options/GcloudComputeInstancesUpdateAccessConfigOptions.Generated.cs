...
public record GcloudComputeInstancesUpdateAccessConfigOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// update a Compute Engine     virtual machine access configuration
    /// </summary>
    ...
    public GcloudComputeInstancesUpdateAccessConfigOptions(
        string InstanceName
    )
    {
    ...
    /// <summary>
    /// At most one of these can be specified: If provided, the default DNS PTR record will replace the existing one for external IPv6 in the IPv6 access configuration. Mutually exclusive with ipv6-public-ptr-domain.
    /// </summary>
    [CliFlag("--no-ipv6-public-ptr")]
    public bool? NoIpv6PublicPtr { get; set; }

    /// <summary>
    /// At most one of these can be specified: Assigns a custom PTR domain for the external IPv6 in the access configuration. Mutually exclusive with no-ipv6-public-ptr. This option can only be specified for the default network interface, nic0.
    /// </summary>
