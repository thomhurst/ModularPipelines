...
public record GcloudIamWorkforcePoolsProvidersUpdateOidcOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// update an OIDC workforce     pool provider
    /// </summary>
    /// <param name="Provider">Workforce pool provider resource - The workforce pool provider to update. The arguments in this group can be used to specify the attributes of this resource. This must be specified. ID of the workforce pool provider or fully qualified identifier for the workforce pool provider. To set the provider attribute: ▸ provide the argument provider on the command line. This positional argument must be specified if any of the other arguments in this group are specified.</param>
    public GcloudIamWorkforcePoolsProvidersUpdateOidcOptions(
        string Provider
    )
    {
    ...
    public GcloudIamWorkforcePoolsProvidersUpdateOidcWebSsoResponseType? WebSsoResponseType { get; set; }

    /// <summary>
    /// At most one of these can be specified: Clear the OIDC client secret.
    /// </summary>
    [CliFlag("--clear-client-secret")]
    public bool? ClearClientSecret { get; set; }

    /// <summary>
    /// At most one of these can be specified: The OIDC client secret. Required to enable Authorization Code flow for web sign-in.
    /// </summary>
    [SecretValue]
    ...
    /// <summary>
    /// At most one of these can be specified: Clear the extended attributes configuration.
    /// </summary>
    [CliFlag("--clear-extended-attributes-config")]
    public bool? ClearExtendedAttributesConfig { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: The OAuth 2.0 client ID for retrieving extended attributes from the identity provider. Required to get extended group memberships for a subset of Google Cloud products. The --extended-attributes-client-id flag is restricted. We suggest you use SCIM (https://docs.cloud.google.com/iam/docs/configure-scim-oidc-saml) instead.
    /// </summary>
    ...
    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: Represents the identity provider and type of claims that should be fetched. The --extended-attributes-type flag is restricted. We suggest you use SCIM (https://docs.cloud.google.com/iam/docs/configure-scim-oidc-saml) instead. EXTENDED_ATTRIBUTES_TYPE must be (only one value is supported): azure-ad-groups-id.
    /// </summary>
    [CliOption("--extended-attributes-type", Format = OptionFormat.EqualsSeparated)]
    public string? ExtendedAttributesType { get; set; }

    /// <summary>
    /// At most one of these can be specified: Clear the extra attributes configuration.
    /// </summary>
    [CliFlag("--clear-extra-attributes-config")]
    public bool? ClearExtraAttributesConfig { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: The OAuth 2.0 client ID for retrieving extra attributes from the identity provider. Required to get the access token using client credentials grant flow.
    /// </summary>
    [CliOption("--extra-attributes-client-id", Format = OptionFormat.EqualsSeparated)]
    public string? ExtraAttributesClientId { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: The OAuth 2.0 client secret for retrieving extra attributes from the identity provider. Required to get the access token using client credentials grant flow.
    /// </summary>
    [SecretValue]
    ...
    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: OIDC identity provider's issuer URI. Must be a valid URI using the https scheme. Required to get the OIDC discovery document.
    /// </summary>
    [CliOption("--extra-attributes-issuer-uri", Format = OptionFormat.EqualsSeparated)]
    public string? ExtraAttributesIssuerUri { get; set; }

    /// <summary>
    /// At most one of these can be specified: Or at least one of these can be specified: Represents the identity provider and type of claims that should be fetched. EXTRA_ATTRIBUTES_TYPE must be one of: attributes-type-unspecified, azure-ad-groups-display-name, azure-ad-groups-id, azure-ad-groups-mail.
    /// </summary>
    [CliOption("--extra-attributes-type", Format = OptionFormat.EqualsSeparated)]
    public GcloudIamWorkforcePoolsProvidersUpdateOidcExtraAttributesType? ExtraAttributesType { get; set; }

    /// <summary>
    /// At most one of these can be specified: Clear the SCIM usage configuration, resetting it to unspecified.
    /// </summary>
    [CliFlag("--clear-scim-usage")]
    public bool? ClearScimUsage { get; set; }

    /// <summary>
    /// At most one of these can be specified: Specifies whether the workforce identity pool provider uses SCIM-managed groups instead of the google.groups attribute mapping for authorization checks. The scim_usage and extended_attributes_oauth2_client fields are mutually exclusive. A request that enables both fields on the same workforce identity pool provider will produce an error. Use enabled-for-groups to enable SCIM-managed groups. Use enabled-for-users-groups to enable SCIM-managed user claims and groups. Use scim-usage-unspecified to disable SCIM-managed groups. SCIM_USAGE must be one of: enabled-for-groups, enabled-for-users-groups, scim-usage-unspecified.
    /// </summary>
    [CliOption("--scim-usage", Format = OptionFormat.EqualsSeparated)]
    public GcloudIamWorkforcePoolsProvidersUpdateOidcScimUsage? ScimUsage { get; set; }
    ...
    {
        if ((ClearClientSecret == true ? 1 : 0) + (!string.IsNullOrWhiteSpace(ClientSecretValue) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of ClearClientSecret or ClientSecretValue may be specified.", [nameof(ClearClientSecret), nameof(ClientSecretValue)]);
        }
        if ((ClearExtendedAttributesConfig == true ? 1 : 0) + ((!string.IsNullOrWhiteSpace(ExtendedAttributesClientId) || !string.IsNullOrWhiteSpace(ExtendedAttributesClientSecretValue) || !string.IsNullOrWhiteSpace(ExtendedAttributesFilter) || !string.IsNullOrWhiteSpace(ExtendedAttributesIssuerUri) || !string.IsNullOrWhiteSpace(ExtendedAttributesType)) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of ClearExtendedAttributesConfig or (ExtendedAttributesClientId, ExtendedAttributesClientSecretValue, ExtendedAttributesFilter, ExtendedAttributesIssuerUri, or ExtendedAttributesType) may be specified.", [nameof(ClearExtendedAttributesConfig), nameof(ExtendedAttributesClientId), nameof(ExtendedAttributesClientSecretValue), nameof(ExtendedAttributesFilter), nameof(ExtendedAttributesIssuerUri), nameof(ExtendedAttributesType)]);
        }
        if ((ClearExtraAttributesConfig == true ? 1 : 0) + ((!string.IsNullOrWhiteSpace(ExtraAttributesClientId) || !string.IsNullOrWhiteSpace(ExtraAttributesClientSecretValue) || !string.IsNullOrWhiteSpace(ExtraAttributesFilter) || !string.IsNullOrWhiteSpace(ExtraAttributesIssuerUri) || (object?)ExtraAttributesType is not null) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of ClearExtraAttributesConfig or (ExtraAttributesClientId, ExtraAttributesClientSecretValue, ExtraAttributesFilter, ExtraAttributesIssuerUri, or ExtraAttributesType) may be specified.", [nameof(ClearExtraAttributesConfig), nameof(ExtraAttributesClientId), nameof(ExtraAttributesClientSecretValue), nameof(ExtraAttributesFilter), nameof(ExtraAttributesIssuerUri), nameof(ExtraAttributesType)]);
        }
        if ((ClearScimUsage == true ? 1 : 0) + ((object?)ScimUsage is not null ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of ClearScimUsage or ScimUsage may be specified.", [nameof(ClearScimUsage), nameof(ScimUsage)]);
        }
        yield break;
    }
