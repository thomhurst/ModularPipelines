...
public record GcloudColabExecutionsCreateOptions : GcloudOptions, IValidatableObject
{
    ...
    {
        if ((!string.IsNullOrWhiteSpace(DirectContent) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(DataformRepositoryName)) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(CommitSha)) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(GcsNotebookUri) || !string.IsNullOrWhiteSpace(Generation)) ? 1 : 0) != 1)
        {
            yield return new ValidationResult("Exactly one of DirectContent, (DataformRepositoryName), (CommitSha), or (GcsNotebookUri or Generation) must be specified.", [nameof(DirectContent), nameof(DataformRepositoryName), nameof(CommitSha), nameof(GcsNotebookUri), nameof(Generation)]);
        }
