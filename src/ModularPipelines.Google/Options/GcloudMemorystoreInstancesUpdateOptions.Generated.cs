...
public record GcloudMemorystoreInstancesUpdateOptions : GcloudOptions, IValidatableObject
{
    ...
    {
        if (((!string.IsNullOrWhiteSpace(AclPolicy)) ? 1 : 0) + ((ClearAclPolicy == true) ? 1 : 0) > 1)
        {
        ...
        if ((ClearPrimaryInstance == true ? 1 : 0) + ((!string.IsNullOrWhiteSpace(PrimaryInstance)) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of ClearPrimaryInstance or (PrimaryInstance) may be specified.", [nameof(ClearPrimaryInstance), nameof(PrimaryInstance)]);
        }
