...
) : GcloudOptions
{
    ...
    /// <summary>
    /// (DEPRECATED) Enable Cloud Operations for GKE.     The --enable-stackdriver-kubernetes flag is deprecated and will be     removed in an upcoming release. Please use --logging and --monitoring     instead. For more information, please read:     https://cloud.google.com/kubernetes-engine/docs/concepts/about-logs and     https://cloud.google.com/kubernetes-engine/docs/how-to/configure-metrics.    Flags for vertical pod autoscaling:
    /// </summary>
    ...
    /// <summary>
    /// Set the components that have logging enabled. Valid component values     are: SYSTEM, WORKLOAD, API_SERVER, CONTROLLER_MANAGER, SCHEDULER,     KCP_HPA, NONE     For more information, see     https://cloud.google.com/kubernetes-engine/docs/concepts/about-logs#available-logs     Examples:       $ gcloud container clusters create --logging=SYSTEM       $ gcloud container clusters create \         --logging=SYSTEM,API_SERVER,WORKLOAD       $ gcloud container clusters create --logging=NONE
    /// </summary>
    ...
    /// <summary>
    /// Configure the Provisioned Throughput for the node pool boot disks. Only     valid for hyperdisk-balanced boot disks.    ClusterDNS
    /// </summary>
    ...
    /// <summary>
    /// DNS provider to use for this cluster. CLUSTER_DNS must be one of:      clouddns       Selects Cloud DNS as the DNS provider for the cluster.     default       Selects the default DNS provider (kube-dns) for the cluster.     kubedns       Selects Kube DNS as the DNS provider for the cluster.
    /// </summary>
    ...
    /// <summary>
    /// Maximum number of nodes per zone in the node pool.     Maximum number of nodes per zone to which the node pool specified by     --node-pool (or default node pool if unspecified) can scale. Ignored     unless --enable-autoscaling is also specified.
    /// </summary>
    [CliOption("--max-nodes", Format = OptionFormat.EqualsSeparated)]
    public string? MaxNodes { get; set; }

    /// <summary>
    /// Minimum number of nodes per zone in the node pool.     Minimum number of nodes per zone to which the node pool specified by     --node-pool (or default node pool if unspecified) can scale. Ignored     unless --enable-autoscaling is also specified.
    /// </summary>
    [CliOption("--min-nodes", Format = OptionFormat.EqualsSeparated)]
    public string? MinNodes { get; set; }

    /// <summary>
    /// Maximum number of all nodes in the node pool.     Maximum number of all nodes to which the node pool specified by     --node-pool (or default node pool if unspecified) can scale. Ignored     unless --enable-autoscaling is also specified.
    /// </summary>
    [CliOption("--total-max-nodes", Format = OptionFormat.EqualsSeparated)]
    public string? TotalMaxNodes { get; set; }

    /// <summary>
    /// Minimum number of all nodes in the node pool.     Minimum number of all nodes to which the node pool specified by     --node-pool (or default node pool if unspecified) can scale. Ignored     unless --enable-autoscaling is also specified.
    /// </summary>
    ...
    /// <summary>
    /// Set the minimum interval of time between patch version cluster     upgrades.    One of either maintenance-window or the group of maintenance-window flags   can be set.    At most one of these can be specified:     --maintenance-window=START_TIME      Set a time of day when you prefer maintenance to start on this      cluster. For example:        $ gcloud container clusters create example-cluster \          --maintenance-window=12:43      The time corresponds to the UTC time zone, and must be in HH:MM      format.      Non-emergency maintenance will occur in the 4 hour block starting at      the specified time.      This is mutually exclusive with the recurring maintenance windows and      will overwrite any existing window. Compatible with maintenance      exclusions.     Or at least one of these can be specified:      Set a flexible maintenance window by specifying a window that recurs     per an RFC 5545 RRULE. Non-emergency maintenance will occur in the     recurring windows.      Examples:      For a 9-5 Mon-Wed UTC-4 maintenance window:        $ gcloud container clusters create example-cluster \         --maintenance-window-start=2000-01-01T09:00:00-04:00 \         --maintenance-window-end=2000-01-01T17:00:00-04:00 \         --maintenance-window-recurrence='FREQ=WEEKLY;BYDAY=MO,TU,WE'      For a daily window from 22:00 - 04:00 UTC:        $ gcloud container clusters create example-cluster \         --maintenance-window-start=2000-01-01T22:00:00Z \         --maintenance-window-end=2000-01-02T04:00:00Z \         --maintenance-window-recurrence=FREQ=DAILY      --maintenance-window-recurrence=RRULE       An RFC 5545 RRULE, specifying how the window will recur. Note that       minimum requirements for maintenance periods will be enforced. Note       that FREQ=SECONDLY, MINUTELY, and HOURLY are not supported.       This flag argument must be specified if any of the other arguments       in this group are specified.      --maintenance-window-start=TIME_STAMP       Start time of the first window (can occur in the past). The start       time influences when the window will start for recurrences. See $       gcloud topic datetimes for information on time formats.       This flag argument must be specified if any of the other arguments       in this group are specified.      Exactly one of these must be specified:       --maintenance-window-duration=PT1H23M45S        The duration of maintenance windows, expressed as an ISO 8601        duration (https://en.wikipedia.org/wiki/ISO_8601#Durations) in        hours, minutes, and seconds. You must set a maintenance window to        at least 4 hours (4H). Use this flag or the        --maintenance-window-end flag for the duration of the maintenance        window.       --maintenance-window-end=TIME_STAMP        The end time for calculating the duration of the maintenance        window, as expressed by the amount of time after the START_TIME,        in the same format. The value for END_TIME must be in the future,        relative to START_TIME. This only calculates the duration of the        window, and doesn't set when the maintenance window stops        recurring. Maintenance windows only stop recurring when they're        removed. See $ gcloud topic datetimes for information on time        formats.        This flag argument must be specified if any of the other        arguments in this group are specified.    Basic auth
    /// </summary>
    ...
    /// <summary>
    /// The type of the reservation for the default initial node pool.     RESERVATION_AFFINITY must be one of: any, none, specific,     any-reservation-then-fail.    Options to specify the node identity.    Scopes options.
    /// </summary>
