...
public record GcloudContainerClustersCreateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// Enable the cost allocation feature. When enabled, you can get informational GKE cost allocation by cluster, namespace and label in your billing data exported to BigQuery (https://cloud.google.com/billing/docs/how-to/export-data-bigquery).
    /// </summary>
    ...
    /// <summary>
    /// Labels to apply to the Google Cloud resources in use by the Kubernetes Engine cluster. These are unrelated to Kubernetes labels. Examples: $ gcloud container clusters create example-cluster \ --labels=label_a=value1,label_b=,label_c=value3 Collection entries are joined with commas into one option value. For entries containing commas, supply one pre-escaped list value using gcloud topic escaping (https://cloud.google.com/sdk/gcloud/reference/topic/escaping).
    /// </summary>
    ...
    /// <summary>
    /// Same as --metadata except that the value for the entry will be read from a local file. Collection entries are joined with commas into one option value. For entries containing commas, supply one pre-escaped list value using gcloud topic escaping (https://cloud.google.com/sdk/gcloud/reference/topic/escaping).
    /// </summary>
    ...
    /// <summary>
    /// DNS scope for the Cloud DNS zone created - valid only with --cluster-dns=clouddns. Defaults to cluster. CLUSTER_DNS_SCOPE must be one of: cluster Configures the Cloud DNS zone to be private to the cluster. vpc Configures the Cloud DNS zone to be private to the VPC Network.
    /// </summary>
