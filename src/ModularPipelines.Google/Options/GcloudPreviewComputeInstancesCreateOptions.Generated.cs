...
public record GcloudPreviewComputeInstancesCreateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// At most one of these can be specified: Creates a DNS PTR record for the external IP of the instance.
    /// </summary>
    [CliFlag("--public-ptr")]
    public bool? PublicPtr { get; set; }

    /// <summary>
    /// At most one of these can be specified: If provided, no DNS PTR record is created for the external IP of the instance. Mutually exclusive with public-ptr-domain.
    /// </summary>
    [CliFlag("--no-public-ptr")]
    public bool? NoPublicPtr { get; set; }

    /// <summary>
    /// At most one of these can be specified: Assigns a custom PTR domain for the external IP of the instance. Mutually exclusive with no-public-ptr.
    /// </summary>
    [CliOption("--public-ptr-domain", Format = OptionFormat.EqualsSeparated)]
    public string? PublicPtrDomain { get; set; }

    /// <summary>
    /// At most one of these can be specified: If both this flag and --public-ptr are specified, creates a DNS PTR record for the external IP of the instance with the PTR domain name being the DNS name of the instance.
    /// </summary>
    ...
    IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
    {
        ...
        if ((!string.IsNullOrWhiteSpace(CustomCpu) || !string.IsNullOrWhiteSpace(CustomMemory) || CustomExtensions == true || !string.IsNullOrWhiteSpace(CustomVmType) || (object?)ImageFamilyScope is not null || !string.IsNullOrWhiteSpace(ImageProject)) && (!(!string.IsNullOrWhiteSpace(CustomCpu))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(CustomCpu) || !string.IsNullOrWhiteSpace(CustomMemory) || CustomExtensions == true || !string.IsNullOrWhiteSpace(CustomVmType) || (object?)ImageFamilyScope is not null || !string.IsNullOrWhiteSpace(ImageProject)) && (!(!string.IsNullOrWhiteSpace(CustomMemory))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(InstanceKmsKey) || !string.IsNullOrWhiteSpace(InstanceKmsKeyring) || !string.IsNullOrWhiteSpace(InstanceKmsLocation) || !string.IsNullOrWhiteSpace(InstanceKmsProject)) && (!(!string.IsNullOrWhiteSpace(InstanceKmsKey))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(Node) ? 1 : 0) + (!string.IsNullOrWhiteSpace(NodeAffinityFile) ? 1 : 0) + (!string.IsNullOrWhiteSpace(NodeGroup) ? 1 : 0) > 1)
        {
