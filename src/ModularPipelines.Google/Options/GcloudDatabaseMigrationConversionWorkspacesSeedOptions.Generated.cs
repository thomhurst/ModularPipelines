...
public record GcloudDatabaseMigrationConversionWorkspacesSeedOptions : GcloudOptions, IValidatableObject
{
    ...
    {
        if (((!string.IsNullOrWhiteSpace(DestinationConnectionProfile)) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(GcsPath)) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(SourceConnectionProfile)) ? 1 : 0) != 1)
        {
            yield return new ValidationResult("Exactly one of (DestinationConnectionProfile), (GcsPath), or (SourceConnectionProfile) must be specified.", [nameof(DestinationConnectionProfile), nameof(GcsPath), nameof(SourceConnectionProfile)]);
        }
        if ((!string.IsNullOrWhiteSpace(DestinationConnectionProfile) || !string.IsNullOrWhiteSpace(GcsPath) || !string.IsNullOrWhiteSpace(SourceConnectionProfile)) && (!string.IsNullOrWhiteSpace(SourceConnectionProfile)) && (!(!string.IsNullOrWhiteSpace(SourceConnectionProfile))))
        {
