...
public record GcloudComputeTpusQueuedResourcesSshOptions : GcloudOptions
{
    ...
    /// <summary>
    /// Command to run on the Cloud TPU VM. Runs the command on the target Cloud TPU Queued Resource's nodes and then exits. Note: in the case of a TPU Pod, it will only run the command in the workers specified with the --worker flag (defaults to worker all if not set). This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    [CliOption("--command", Format = OptionFormat.EqualsSeparated)]
    public string? Command { get; set; }

    /// <summary>
    /// Path to the directory to output the logs of the commands. The path can be relative or absolute. The directory must already exist. If not specified, standard output will be used. The logs will be written in files named {WORKER_ID}.log. For example: "2.log".
    /// </summary>
