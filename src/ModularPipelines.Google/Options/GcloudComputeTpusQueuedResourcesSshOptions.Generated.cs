...
public record GcloudComputeTpusQueuedResourcesSshOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// SSH into a Cloud TPU Queued     Resource's node(s)
    /// </summary>
    /// <param name="UserQr">Specifies the Cloud TPU Queued Resource to send SSH command to. USER specifies the username with which to SSH. If omitted, the user login name is used. QR specifies the name of the Cloud TPU Queued Resource to send SSH command to.</param>
    public GcloudComputeTpusQueuedResourcesSshOptions(
        string UserQr
    )
    {
        global::System.ArgumentNullException.ThrowIfNull(UserQr);
        this.UserQr = UserQr;
    }

    public void Deconstruct(out string UserQr)
    {
        UserQr = this.UserQr;
    }
    ...
    /// <summary>
    /// TPU node(s) to connect to. The supported value is a single 0-based index of the node(s) in the case of a TPU Pod. When also using the --command flag, it additionally supports a comma-separated list (e.g. '1,4,6'), range (e.g. '1-3'), or special keyword ``all" to run the command concurrently on each of the specified node(s). Note that when targeting multiple nodes, you should run 'ssh-add' with your private key prior to executing the gcloud command. Default: 'ssh-add ~/.ssh/google_compute_engine'.
    /// </summary>
    ...
    public GcloudComputeTpusQueuedResourcesSshStrictHostKeyChecking? StrictHostKeyChecking { get; set; }
    ...
    /// <summary>
    /// These arguments are used to run commands using SSH. Command to run on the Cloud TPU VM. Runs the command on the target Cloud TPU Queued Resource's nodes and then exits. Note: in the case of a TPU Pod, it will only run the command in the workers specified with the --worker flag (defaults to worker all if not set). This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    [CliOption("--command", Format = OptionFormat.EqualsSeparated)]
    public string? Command { get; set; }

    /// <summary>
    /// These arguments are used to run commands using SSH. Path to the directory to output the logs of the commands. The path can be relative or absolute. The directory must already exist. If not specified, standard output will be used. The logs will be written in files named {WORKER_ID}.log. For example: "2.log".
    /// </summary>
    [CliOption("--output-directory", Format = OptionFormat.EqualsSeparated)]
    public string? OutputDirectory { get; set; }

    /// <summary>
    /// These arguments are used to run commands using SSH. At most one of these can be specified: Connect to TPU VMs using their internal IP addresses rather than their external IP addresses. Use this to connect from a Google Compute Engine VM to a TPU VM on the same VPC network, or between two peered VPC networks.
    /// </summary>
    [CliFlag("--internal-ip")]
    public bool? InternalIp { get; set; }

    /// <summary>
    /// These arguments are used to run commands using SSH. At most one of these can be specified: Tunnel the SSH connection through Cloud Identity-Aware Proxy for TCP forwarding. This flag must be specified to attempt to connect via IAP tunneling. If it is not set, and connection to a Cloud TPU VM without external IP address is attempted from outside the network, then the command will fail. To use IAP tunneling, there must be firewall access to the SSH port for the IAP TCP IP address range for the network the TPU is created in. See the user guide (https://cloud.google.com/iap/docs/using-tcp-forwarding) for more details. To learn more, see the IAP for TCP forwarding documentation (https://cloud.google.com/iap/docs/tcp-forwarding-overview).
    /// </summary>
    ...
    /// <summary>
    /// Specifies the Cloud TPU Queued Resource to send SSH command to. USER specifies the username with which to SSH. If omitted, the user login name is used. QR specifies the name of the Cloud TPU Queued Resource to send SSH command to.
    /// </summary>
    [CliArgument(0, Phase = CommandLinePhase.EarlyOperand, Required = true)]
    public string UserQr { get; private init; }

    /// <summary>
    /// Flags and positionals passed to the underlying ssh implementation. The '--' argument must be specified between gcloud specific args on the left and SSH_ARGS on the right. Example: $ gcloud compute tpus queued-resources ssh example-instance \ --zone=us-central1-a -- -vvv -L 80:%TPU%:80
    /// </summary>
    ...
    {
        if ((!string.IsNullOrWhiteSpace(Command) || !string.IsNullOrWhiteSpace(OutputDirectory) || InternalIp == true || TunnelThroughIap == true || !string.IsNullOrWhiteSpace(SshKeyExpiration) || !string.IsNullOrWhiteSpace(SshKeyExpireAfter)) && (!(!string.IsNullOrWhiteSpace(Command))))
        {
            yield return new ValidationResult("Command must be specified when other arguments in this group are specified.", [nameof(Command)]);
        }
        if ((!string.IsNullOrWhiteSpace(Command) || !string.IsNullOrWhiteSpace(OutputDirectory) || InternalIp == true || TunnelThroughIap == true || !string.IsNullOrWhiteSpace(SshKeyExpiration) || !string.IsNullOrWhiteSpace(SshKeyExpireAfter)) && ((InternalIp == true ? 1 : 0) + (TunnelThroughIap == true ? 1 : 0) > 1))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(Command) || !string.IsNullOrWhiteSpace(OutputDirectory) || InternalIp == true || TunnelThroughIap == true || !string.IsNullOrWhiteSpace(SshKeyExpiration) || !string.IsNullOrWhiteSpace(SshKeyExpireAfter)) && ((!string.IsNullOrWhiteSpace(SshKeyExpiration) ? 1 : 0) + (!string.IsNullOrWhiteSpace(SshKeyExpireAfter) ? 1 : 0) > 1))
        {
