...
public record GcloudAssetQueryOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// Specifies what time period or point in time to query asset metadata at. At most one of these can be specified: Timestamp to take a snapshot on assets. This can only be a current or past time. If not specified, the current time will be used. Due to delays in resource data collection and indexing, there is a volatile window during which running the same query at different times may return different results. See $ gcloud topic datetimes for information on time formats.
    /// </summary>
    [CliOption("--snapshot-time", Format = OptionFormat.EqualsSeparated)]
    public string? SnapshotTime { get; set; }

    /// <summary>
    /// Specifies what time period or point in time to query asset metadata at. At most one of these can be specified: Or at least one of these can be specified: Specifies what time period or point in time to query asset metadata at. Start time of the time window (inclusive) for the asset history. Must be after the current time minus 35 days. See $ gcloud topic datetimes for information on time formats. This flag argument must be specified if any of the other arguments in this group are specified.
    /// </summary>
    ...
    IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
    {
        ...
        if ((!string.IsNullOrWhiteSpace(SnapshotTime) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(StartTime) || !string.IsNullOrWhiteSpace(EndTime)) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of SnapshotTime or (StartTime or EndTime) may be specified.", [nameof(SnapshotTime), nameof(StartTime), nameof(EndTime)]);
        }
        if ((!string.IsNullOrWhiteSpace(SnapshotTime) || !string.IsNullOrWhiteSpace(StartTime) || !string.IsNullOrWhiteSpace(EndTime)) && (!string.IsNullOrWhiteSpace(StartTime) || !string.IsNullOrWhiteSpace(EndTime)) && (!string.IsNullOrWhiteSpace(StartTime) || !string.IsNullOrWhiteSpace(EndTime)) && (!(!string.IsNullOrWhiteSpace(StartTime))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(BigqueryTable) || !string.IsNullOrWhiteSpace(BigqueryDataSet)) && (!(!string.IsNullOrWhiteSpace(BigqueryTable))))
        {
