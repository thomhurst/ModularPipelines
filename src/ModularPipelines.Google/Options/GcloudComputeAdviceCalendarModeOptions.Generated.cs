...
public record GcloudComputeAdviceCalendarModeOptions : GcloudOptions, IValidatableObject
{
    ...
    {
        if (((object?)ChipCount is not null ? 1 : 0) + (!string.IsNullOrWhiteSpace(TpuVersion) ? 1 : 0) + (!string.IsNullOrWhiteSpace(WorkloadType) ? 1 : 0) + (!string.IsNullOrWhiteSpace(MachineType) ? 1 : 0) + ((object?)VmCount is not null ? 1 : 0) + (!string.IsNullOrWhiteSpace(LocalSsd) ? 1 : 0) != 1)
        {
