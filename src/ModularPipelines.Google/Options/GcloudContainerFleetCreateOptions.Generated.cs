...
public record GcloudContainerFleetCreateOptions : GcloudOptions
{
    ...
    /// <summary>
    /// Default cluster configurations to apply across the fleet. Binary Authorization config. Security posture config. List of label KEY=VALUE pairs to add. Keys must start with a lowercase character and contain only hyphens (-), underscores (_), lowercase characters, and numbers. Values must contain only hyphens (-), underscores (_), lowercase characters, and numbers.
    /// </summary>
    [CliOption("--labels", Format = OptionFormat.EqualsSeparated)]
    public IReadOnlyList<KeyValue>? Labels { get; set; }

    /// <summary>
    /// Default cluster configurations to apply across the fleet. Binary Authorization config. Security posture config. Configure binary authorization mode for clusters to onboard the fleet, $ gcloud container fleet create \ --binauthz-evaluation-mode=policy-bindings BINAUTHZ_EVALUATION_MODE must be one of: disabled, policy-bindings.
    /// </summary>
    ...
    /// <summary>
    /// Default cluster configurations to apply across the fleet. Binary Authorization config. Security posture config. To apply standard security posture to clusters in the fleet, $ gcloud container fleet create --security-posture=standard SECURITY_POSTURE must be one of: disabled, standard, enterprise.
    /// </summary>
    [CliOption("--security-posture", Format = OptionFormat.EqualsSeparated)]
    public GcloudSecurityPosture? SecurityPosture { get; set; }

    /// <summary>
    /// Default cluster configurations to apply across the fleet. Binary Authorization config. Security posture config. To apply standard vulnerability scanning to clusters in the fleet, $ gcloud container fleet create \ --workload-vulnerability-scanning=standard WORKLOAD_VULNERABILITY_SCANNING must be one of: disabled, standard, enterprise.
    /// </summary>
