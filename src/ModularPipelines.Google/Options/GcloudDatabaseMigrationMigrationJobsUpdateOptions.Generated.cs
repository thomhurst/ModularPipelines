...
public record GcloudDatabaseMigrationMigrationJobsUpdateOptions : GcloudOptions
{
    ...
    /// <summary>
    /// List of label KEY=VALUE pairs to update. If a label exists, its value     is modified. Otherwise, a new label is created.     Keys must start with a lowercase character and contain only hyphens     (-), underscores (_), lowercase characters, and numbers. Values must     contain only hyphens (-), underscores (_), lowercase characters, and     numbers.    The migration job objects config.    The migration job objects config for databases.    At most one of these can be specified:     --all-databases      Migrate all databases for the migration job. This flag is used only      for Postgres to AlloyDB, and Postgres to Cloud SQL Postgres and MySQL      to Cloud SQL MySQL migrations.     --databases-filter=databaseName,[...]      A list of databases to be migrated to the destination instance.      Provide databases as a comma separated list. This flag is used only      for Postgres to AlloyDB, and Postgres to Cloud SQL Postgres and MySQL      to Cloud SQL MySQL migrations.    At most one of these can be specified:     --clear-labels      Remove all labels. If --update-labels is also specified then      --clear-labels is applied first.      For example, to remove all labels:        $ gcloud database-migration migration-jobs update --clear-labels      To remove all existing labels and create two new labels, foo and baz:        $ gcloud database-migration migration-jobs update --clear-labels \         --update-labels foo=bar,baz=qux     --remove-labels=[KEY,...]      List of label keys to remove. If a label does not exist it is      silently ignored. If --update-labels is also specified then      --update-labels is applied first.    At most one of these can be specified:     --dump-flags=[KEY=VALUE,...]      A list of dump flags. An object containing a list of "key": "value"      pairs.     --dump-path=DUMP_PATH      Path to the dump file in Google Cloud Storage, in the format:      gs://[BUCKET_NAME]/[OBJECT_NAME].    The heterogeneous migration config. This is used only for Oracle to Cloud   SQL for PostgreSQL and SQL Server to Cloud SQL for PostgreSQL migrations.    Configuration for Oracle or SQL Server as a source in a heterogeneous   migration.
    /// </summary>
    ...
    /// <summary>
    /// Maximum number of connections Database Migration Service will open to     the source for full dump phase.    Configuration for Postgres as a destination in a heterogeneous migration.
    /// </summary>
