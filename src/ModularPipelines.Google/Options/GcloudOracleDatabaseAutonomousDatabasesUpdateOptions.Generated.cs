...
public record GcloudOracleDatabaseAutonomousDatabasesUpdateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// Arguments for the refreshable clone. The encryption key used to encrypt the Autonomous Database. All arguments needed to update encryption_key_kms. Update labels. At most one of these can be specified: Or at least one of these can be specified: At most one of these can be specified: Clear labels value and set to empty map.
    /// </summary>
    [CliFlag("--clear-labels")]
    public bool? ClearLabels { get; set; }

    /// <summary>
    /// Arguments for the refreshable clone. The encryption key used to encrypt the Autonomous Database. All arguments needed to update encryption_key_kms. Update labels. At most one of these can be specified: Or at least one of these can be specified: At most one of these can be specified: Remove existing value from map labels. Sets remove_labels value. Shorthand Example: --remove-labels=string,string JSON Example: --remove-labels=["string"] File Example: --remove-labels=path_to_file.(yaml|json)
    /// </summary>
    ...
    {
        if ((ClearEncryptionKeyKms == true ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of ClearEncryptionKeyKms may be specified.", [nameof(ClearEncryptionKeyKms)]);
        }
