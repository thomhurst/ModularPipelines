...
public record GcloudPreviewComputeInstanceTemplatesCreateOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// create a Compute Engine     virtual machine instance template
    /// </summary>
    /// <param name="Name">Name of the instance template to create.</param>
    public GcloudPreviewComputeInstanceTemplatesCreateOptions(
        string Name
    )
    {
    ...
    public GcloudPreviewComputeInstanceTemplatesCreateNetworkTier? NetworkTier { get; set; }
    ...
    /// <summary>
    /// At most one of these can be specified: Assigns the given external IP address to the instance that is created. This option can only be used when creating a single instance.
    /// </summary>
    ...
    /// <summary>
    /// Key resource - The Cloud KMS (Key Management Service) cryptokey that will be used to protect the disk. The 'Compute Engine Service Agent' service account must hold permission 'Cloud KMS CryptoKey Encrypter/Decrypter'. The arguments in this group can be used to specify the attributes of this resource. Custom machine type extensions. Specifies the reservation for instances created from this template. Labels that you can apply to your service proxy. These will be reflected in your Envoy proxy's bootstrap metadata. These can be any key=value pairs that you want to set as proxy metadata (for example, for use with config filtering). You might use these flags for application and version labels: app=review and/or version=canary. Collection entries are joined with commas into one option value. For entries containing commas, supply one pre-escaped list value using gcloud topic escaping (https://cloud.google.com/sdk/gcloud/reference/topic/escaping).
    /// </summary>
    [CliOption("--service-proxy-labels", Format = OptionFormat.EqualsSeparated, CollectionSeparator = ",")]
    public IReadOnlyList<KeyValue>? ServiceProxyLabels
    {
        get;
