...
public record GcloudRecaptchaKeysAddIpOverrideOptions : GcloudOptions
{
    ...
    /// <summary>
    /// If set to allow, the IP address/CIDR range will be allowlisted for the key. OVERRIDE must be one of: allow, override-type-unspecified.
    /// </summary>
    [CliOption("--override", Format = OptionFormat.EqualsSeparated)]
    public string Override { get; private init; }

    /// <summary>
    /// Key resource - The reCAPTCHA key to add the IP override to. This represents a Cloud resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways. To set the project attribute: ◆ provide the argument key on the command line with a fully specified name; ◆ provide the argument --project on the command line; ◆ set the property core/project. This must be specified. ID of the key or fully qualified identifier for the key. To set the key attribute: ▸ provide the argument key on the command line.
    /// </summary>
