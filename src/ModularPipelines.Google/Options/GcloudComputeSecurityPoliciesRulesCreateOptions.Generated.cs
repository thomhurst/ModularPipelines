...
) : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// Security policy rule matcher. At least one of these must be specified: --network-user-defined-fields=[NAME;VALUE:VALUE:...,...] Each element names a defined field and lists the matching values for that field. The destination IPs/IP ranges to match for this rule. To match all IPs specify *.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. At least one of these must be specified: --network-user-defined-fields=[NAME;VALUE:VALUE:...,...] Each element names a defined field and lists the matching values for that field. BGP Autonomous System Number associated with the source IP address to match for this rule.
    /// </summary>
    [CliOption("--network-src-asns", Format = OptionFormat.EqualsSeparated)]
    public IEnumerable<string>? NetworkSrcAsns { get; set; }

    /// <summary>
    /// Security policy rule matcher. At least one of these must be specified: --network-user-defined-fields=[NAME;VALUE:VALUE:...,...] Each element names a defined field and lists the matching values for that field. The source IPs/IP ranges to match for this rule. To match all IPs specify *.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. At least one of these must be specified: --network-user-defined-fields=[NAME;VALUE:VALUE:...,...] Each element names a defined field and lists the matching values for that field. The two letter ISO 3166-1 alpha-2 country code associated with the source IP address to match for this rule. To match all region codes specify *.
    /// </summary>
    ...
    {
        if (!(!string.IsNullOrWhiteSpace(Expression) || NetworkDestIpRanges?.Any() == true || NetworkDestPorts?.Any() == true || NetworkIpProtocols?.Any() == true || NetworkSrcAsns?.Any() == true || NetworkSrcIpRanges?.Any() == true || NetworkSrcPorts?.Any() == true || NetworkSrcRegionCodes?.Any() == true || SrcIpRanges?.Any() == true))
        {
