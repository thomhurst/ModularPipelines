...
public record GcloudComputeMigrationMachineImageImportsCreateOptions : GcloudOptions, IValidatableObject
{
    /// <summary>
    /// import machine     images to Google Compute Engine
    /// </summary>
    ...
    public GcloudComputeMigrationMachineImageImportsCreateOptions(
        string SourceFile,
    ...
    public GcloudComputeMigrationMachineImageImportsCreateSecureBoot? SecureBoot { get; set; }
    ...
    /// <summary>
    /// OS adaptation parameters. At most one of these can be specified: If true, skip OS adaptation. Default false.
    /// </summary>
    [CliFlag("--skip-os-adaptation")]
    public bool? SkipOsAdaptation { get; set; }

    /// <summary>
    /// OS adaptation parameters. At most one of these can be specified: Or at least one of these can be specified: A map of flags to pass to the OS adaptation process. The actual value depends on the modifier and can also be empty. e.g. --adaptation-modifiers=flag1=value1,flag2
    /// </summary>
    [CliOption("--adaptation-modifiers", Format = OptionFormat.EqualsSeparated)]
    public string? AdaptationModifiers { get; set; }

    /// <summary>
    /// OS adaptation parameters. At most one of these can be specified: Or at least one of these can be specified: This property will trigger an internal process which will convert the image from using the existing boot option to another. The size of the boot disk might be increased to allow the conversion. Currently only support BIOS_TO_EFI. BOOT_CONVERSION must be one of: bios-to-efi, boot-conversion-unspecified, none.
    /// </summary>
    [CliOption("--boot-conversion", Format = OptionFormat.EqualsSeparated)]
    public GcloudComputeMigrationMachineImageImportsCreateBootConversion? BootConversion { get; set; }

    /// <summary>
    /// OS adaptation parameters. At most one of these can be specified: Or at least one of these can be specified: If true, generalize the imported machine image. Default false. The generalization process enables co-existence of multiple VMs created from the same machine image. For Windows, generalizing the machine image removes computer-specific information such as installed drivers and the computer security identifier (SID).
    /// </summary>
    [CliFlag("--generalize")]
    public bool? Generalize { get; set; }

    /// <summary>
    /// OS adaptation parameters. At most one of these can be specified: Or at least one of these can be specified: The license to use post migration. LICENSE_TYPE must be one of: compute-engine-license-type-byol, compute-engine-license-type-default, compute-engine-license-type-payg.
    /// </summary>
    [CliOption("--license-type", Format = OptionFormat.EqualsSeparated)]
    public string? LicenseType { get; set; }

    /// <summary>
    /// OS adaptation parameters. At most one of these can be specified: Or at least one of these can be specified: Identifies the file system to adapt.
    /// </summary>
