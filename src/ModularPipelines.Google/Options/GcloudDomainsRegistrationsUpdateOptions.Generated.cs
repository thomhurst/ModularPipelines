...
// Source: https://cloud.google.com/sdk/gcloud/reference/domains/registrations/update
...
public record GcloudDomainsRegistrationsUpdateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// At most one of these can be specified: Remove all labels. If --update-labels is also specified then --clear-labels is applied first. For example, to remove all labels: $ gcloud domains registrations update --clear-labels To remove all existing labels and create two new labels, foo and baz: $ gcloud domains registrations update --clear-labels \ --update-labels foo=bar,baz=qux
    /// </summary>
    ...
    /// <summary>
    /// Registration resource - The domain registration to update. This represents a Cloud resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways. To set the project attribute: ◆ provide the argument registration on the command line with a fully specified name; ◆ provide the argument --project on the command line; ◆ set the property core/project. To set the location attribute: ◆ provide the argument registration on the command line with a fully specified name; ◆ location is always global. This must be specified. ID of the registration or fully qualified identifier for the registration. To set the registration attribute: ▸ provide the argument registration on the command line.
    /// </summary>
