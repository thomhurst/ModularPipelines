...
public record GcloudDataprocClustersCreateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// Compute Engine options for Dataproc clusters. Specifies a list of resource manager tags to apply to each cluster node (master and worker nodes). Collection entries are joined with commas into one option value. For entries containing commas, supply one pre-escaped list value using gcloud topic escaping (https://cloud.google.com/sdk/gcloud/reference/topic/escaping).
    /// </summary>
    ...
    public IEnumerable<GcloudDataprocClustersCreateMetricSources>? MetricSources { get; set; }
    ...
    IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
    {
        ...
        if ((!string.IsNullOrWhiteSpace(GcePdKmsKey) || !string.IsNullOrWhiteSpace(GcePdKmsKeyKeyring) || !string.IsNullOrWhiteSpace(GcePdKmsKeyLocation) || !string.IsNullOrWhiteSpace(GcePdKmsKeyProject)) && (!(!string.IsNullOrWhiteSpace(GcePdKmsKey))))
        {
        ...
        if ((!string.IsNullOrWhiteSpace(StopExpirationTime) ? 1 : 0) + (!string.IsNullOrWhiteSpace(StopMaxAge) ? 1 : 0) > 1)
        {
