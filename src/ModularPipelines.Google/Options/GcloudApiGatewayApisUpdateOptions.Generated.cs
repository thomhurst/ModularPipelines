...
// Source: https://cloud.google.com/sdk/gcloud/reference/api-gateway/apis/update
...
public record GcloudApiGatewayApisUpdateOptions : GcloudOptions, IValidatableObject
{
    ...
    /// <summary>
    /// At most one of these can be specified: Remove all labels. If --update-labels is also specified then --clear-labels is applied first. For example, to remove all labels: $ gcloud api-gateway apis update --clear-labels To remove all existing labels and create two new labels, foo and baz: $ gcloud api-gateway apis update --clear-labels \ --update-labels foo=bar,baz=qux
    /// </summary>
    ...
    /// <summary>
    /// Api resource - Name for API which updated. This represents a Cloud resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways. To set the project attribute: ◆ provide the argument api on the command line with a fully specified name; ◆ provide the argument --project on the command line; ◆ set the property core/project. To set the location attribute: ◆ provide the argument api on the command line with a fully specified name; ◆ Location for API and API Configs. Defaults to global. This must be specified. ID of the api or fully qualified identifier for the api. To set the api attribute: ▸ provide the argument api on the command line.
    /// </summary>
