...
) : GcloudOptions
{
    ...
    /// <summary>
    /// Security policy rule matcher. Determines the key name for the rate limit key. Applicable only for the following rate limit key types: ◆ http-header: The name of the HTTP header whose value is taken as the key value. ◆ http-cookie: The name of the HTTP cookie whose value is taken as the key value.
    /// </summary>
    [CliOption("--enforce-on-key-name", Format = OptionFormat.EqualsSeparated)]
    public string? EnforceOnKeyName { get; set; }

    /// <summary>
    /// Security policy rule matcher. Action to take when requests are above the given threshold. When a request is denied, return the specified HTTP response code. When a request is redirected, use the redirect options based on --exceed-redirect-type and --exceed-redirect-target below. EXCEED_ACTION must be one of: deny-403, deny-404, deny-429, deny-502, deny, redirect.
    /// </summary>
    [CliOption("--exceed-action", Format = OptionFormat.EqualsSeparated)]
    public GcloudExceedAction? ExceedAction { get; set; }

    /// <summary>
    /// Security policy rule matcher. URL target for the redirect action that is configured as the exceed action when the redirect type is external-302.
    /// </summary>
    [CliOption("--exceed-redirect-target", Format = OptionFormat.EqualsSeparated)]
    public string? ExceedRedirectTarget { get; set; }

    /// <summary>
    /// Security policy rule matcher. Type for the redirect action that is configured as the exceed action. EXCEED_REDIRECT_TYPE must be one of: google-recaptcha, external-302.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. If specified, the action will not be enforced. Use --preview to enable and --no-preview to disable.
    /// </summary>
    [CliFlag("--preview")]
    public bool? Preview { get; set; }

    /// <summary>
    /// Security policy rule matcher. Number of HTTP(S) requests for calculating the threshold for rate limiting requests.
    /// </summary>
    [CliOption("--rate-limit-threshold-count", Format = OptionFormat.EqualsSeparated)]
    public int? RateLimitThresholdCount { get; set; }

    /// <summary>
    /// Security policy rule matcher. Interval over which the threshold for rate limiting requests is computed.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. A comma-separated list of site keys to be used during the validation of reCAPTCHA action-tokens. The provided site keys need to be created from the reCAPTCHA API under the same project where the security policy is created.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. A comma-separated list of site keys to be used during the validation of reCAPTCHA session-tokens. The provided site keys need to be created from the reCAPTCHA API under the same project where the security policy is created.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. URL target for the redirect action. Must be specified if the redirect type is external-302. Cannot be specified if the redirect type is google-recaptcha.
    /// </summary>
    [CliOption("--redirect-target", Format = OptionFormat.EqualsSeparated)]
    public string? RedirectTarget { get; set; }

    /// <summary>
    /// Security policy rule matcher. Type for the redirect action. Default to external-302 if unspecified while --redirect-target is given. REDIRECT_TYPE must be one of: google-recaptcha, external-302.
    /// </summary>
    [CliOption("--redirect-type", Format = OptionFormat.EqualsSeparated)]
    public GcloudRedirectType? RedirectType { get; set; }

    /// <summary>
    /// Security policy rule matcher. Region of the security policy to update. If not specified, you might be prompted to select a region (interactive mode only). A list of regions can be fetched by running: $ gcloud compute regions list Overrides the default compute/region property value for this command invocation.
    /// </summary>
    [CliOption("--region", Format = OptionFormat.EqualsSeparated)]
    public string? Region { get; set; }

    /// <summary>
    /// Security policy rule matcher. A comma-separated list of header names and header values to add to requests that match this rule.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. The security policy that this rule belongs to.
    /// </summary>
    [CliOption("--security-policy", Format = OptionFormat.EqualsSeparated)]
    public string? SecurityPolicy { get; set; }

    /// <summary>
    /// Security policy rule matcher. The Cloud Armor rules language expression to match for this rule.
    /// </summary>
    ...
    /// <summary>
    /// Security policy rule matcher. Each element names a defined field and lists the matching values for that field.
    /// </summary>
