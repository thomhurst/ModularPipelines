...
public record GcloudDataplexDatascansUpdateDataProfileOptions : GcloudOptions, IValidatableObject
{
    ...
    IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
    {
        ...
        if ((!string.IsNullOrWhiteSpace(DataProfileSpecFile) ? 1 : 0) + ((EnableCatalogPublishing == true || !string.IsNullOrWhiteSpace(ExcludeFieldNames) || !string.IsNullOrWhiteSpace(IncludeFieldNames) || !string.IsNullOrWhiteSpace(Mode) || !string.IsNullOrWhiteSpace(RowFilter) || !string.IsNullOrWhiteSpace(SamplingPercent)) ? 1 : 0) > 1)
        {
            yield return new ValidationResult("At most one of DataProfileSpecFile or (EnableCatalogPublishing, ExcludeFieldNames, IncludeFieldNames, Mode, RowFilter, or SamplingPercent) may be specified.", [nameof(DataProfileSpecFile), nameof(EnableCatalogPublishing), nameof(ExcludeFieldNames), nameof(IncludeFieldNames), nameof(Mode), nameof(RowFilter), nameof(SamplingPercent)]);
        }
