...
public record GcloudRunDeployOptions : GcloudOptions
{
    ...
    /// <summary>
    /// Path to a local YAML file with definitions for all build environment variables. All existing build environment variables will be removed before the new build environment variables are added. Example YAML content: KEY_1: "value1" KEY_2: "value 2"
    /// </summary>
    ...
    /// <summary>
    /// Remove all labels. If --update-labels is also specified then --clear-labels is applied first. For example, to remove all labels: $ gcloud run deploy --clear-labels To remove all existing labels and create two new labels, foo and baz: $ gcloud run deploy --clear-labels --update-labels foo=bar,baz=qux
    /// </summary>
    ...
    /// <summary>
    /// Action type after CMEK key revocation. POST_KEY_REVOCATION_ACTION_TYPE must be one of: prevent-new No new instances will be started after CMEK key revocation. shut-down No new instances will be started and the existing instances will be shut down after CMEK key revocation.
    /// </summary>
