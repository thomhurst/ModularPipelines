...
// Source: https://cloud.google.com/sdk/gcloud/reference/eventarc/pipelines/update
...
public record GcloudEventarcPipelinesUpdateOptions : GcloudOptions
{
    ...
    /// </summary>
    [CliOption("--destinations", Format = OptionFormat.EqualsSeparated)]
    ...
    /// </summary>
    [CliOption("--mediations", Format = OptionFormat.EqualsSeparated)]
    ...
    /// <summary>
    /// At most one of these can be specified: Remove all labels. If --update-labels is also specified then --clear-labels is applied first. For example, to remove all labels: $ gcloud eventarc pipelines update --clear-labels To remove all existing labels and create two new labels, foo and baz: $ gcloud eventarc pipelines update --clear-labels \ --update-labels foo=bar,baz=qux
    /// </summary>
    ...
    /// </summary>
    [CliOption("--input-payload-format-avro-schema-definition", Format = OptionFormat.EqualsSeparated)]
    public string? InputPayloadFormatAvroSchemaDefinition { get; set; }

    /// <summary>
    /// At most one of these can be specified: The pipeline's input payload format is JSON. If this is set, then any messages not matching this format will be treated as persistent errors.
    /// </summary>
    [CliOption("--input-payload-format-json", Format = OptionFormat.EqualsSeparated)]
    public string? InputPayloadFormatJson { get; set; }

    /// <summary>
    /// At most one of these can be specified: The pipeline's input payload Protobuf schema definition. If this is set, then any messages not matching this format will be treated as persistent errors.
    /// </summary>
    [CliOption("--input-payload-format-protobuf-schema-definition", Format = OptionFormat.EqualsSeparated)]
    public string? InputPayloadFormatProtobufSchemaDefinition { get; set; }

    /// <summary>
    /// The retry policy configuration for the pipeline. The pipeline exponentially backs off if the destination is non-responsive or returns a retryable error code. The backoff starts with a 1 second delay and doubles the delay after each failed attempt. The delay is capped at 60 seconds. If the max-retry-delay and min-retry-delay are set to the same value, then the duration between retries is constant. The maximum number of delivery attempts. If not set, the default value is 5.
    /// </summary>
    [CliOption("--max-retry-attempts", Format = OptionFormat.EqualsSeparated)]
    public string? MaxRetryAttempts { get; set; }

    /// <summary>
    /// The retry policy configuration for the pipeline. The pipeline exponentially backs off if the destination is non-responsive or returns a retryable error code. The backoff starts with a 1 second delay and doubles the delay after each failed attempt. The delay is capped at 60 seconds. If the max-retry-delay and min-retry-delay are set to the same value, then the duration between retries is constant. The maximum retry delay in seconds. If not set, the default value is 60.
    /// </summary>
    [CliOption("--max-retry-delay", Format = OptionFormat.EqualsSeparated)]
    public string? MaxRetryDelay { get; set; }

    /// <summary>
    /// The retry policy configuration for the pipeline. The pipeline exponentially backs off if the destination is non-responsive or returns a retryable error code. The backoff starts with a 1 second delay and doubles the delay after each failed attempt. The delay is capped at 60 seconds. If the max-retry-delay and min-retry-delay are set to the same value, then the duration between retries is constant. The minimum retry delay in seconds. If not set, the default value is 1.
    /// </summary>
