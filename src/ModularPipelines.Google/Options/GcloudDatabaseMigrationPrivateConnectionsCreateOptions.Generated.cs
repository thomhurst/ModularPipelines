...
public record GcloudDatabaseMigrationPrivateConnectionsCreateOptions : GcloudOptions, IValidatableObject
{
    ...
    {
        if (((!string.IsNullOrWhiteSpace(NetworkAttachment) || ReservedPublicIp == true || (object?)ReservedPublicIpNatIpsCount is not null) ? 1 : 0) + ((!string.IsNullOrWhiteSpace(Subnet) || !string.IsNullOrWhiteSpace(Vpc)) ? 1 : 0) != 1)
        {
        ...
        if ((!string.IsNullOrWhiteSpace(NetworkAttachment) || ReservedPublicIp == true || (object?)ReservedPublicIpNatIpsCount is not null || !string.IsNullOrWhiteSpace(Subnet) || !string.IsNullOrWhiteSpace(Vpc)) && (!string.IsNullOrWhiteSpace(NetworkAttachment) || ReservedPublicIp == true || (object?)ReservedPublicIpNatIpsCount is not null) && (!(!string.IsNullOrWhiteSpace(NetworkAttachment))))
        {
            yield return new ValidationResult("NetworkAttachment must be specified when other arguments in this group are specified.", [nameof(NetworkAttachment)]);
        }
