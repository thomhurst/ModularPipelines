// <auto-generated>
// This file was generated by ModularPipelines.OptionsGenerator on 2025-12-14.
// Do not edit this file manually.
// </auto-generated>

using ModularPipelines.Context;
using ModularPipelines.Models;
using ModularPipelines.Google.Options;

namespace ModularPipelines.Google.Services;

/// <summary>
/// gcloud developer-connect commands.
/// </summary>
public class GcloudDeveloperConnect
{
    private readonly ICommand _command;
    private GcloudDeveloperConnectConnections? _connections;
    private GcloudDeveloperConnectInsightsConfigs? _insightsConfigs;
    private GcloudDeveloperConnectOperations? _operations;

    /// <summary>
    /// Initializes a new instance of the <see cref="GcloudDeveloperConnect"/> class.
    /// </summary>
    public GcloudDeveloperConnect(ICommand command)
    {
        _command = command;
    }

    #region Sub-command Groups

    /// <summary>
    /// gcloud connections sub-commands.
    /// </summary>
    public GcloudDeveloperConnectConnections Connections => _connections ??= new GcloudDeveloperConnectConnections(_command);

    /// <summary>
    /// gcloud insights-configs sub-commands.
    /// </summary>
    public GcloudDeveloperConnectInsightsConfigs InsightsConfigs => _insightsConfigs ??= new GcloudDeveloperConnectInsightsConfigs(_command);

    /// <summary>
    /// gcloud operations sub-commands.
    /// </summary>
    public GcloudDeveloperConnectOperations Operations => _operations ??= new GcloudDeveloperConnectOperations(_command);

    #endregion

    #region Commands

    /// <summary>
    /// manage Developer Connect resources
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> Execute(
        GcloudDeveloperConnectOptions? options = default,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new GcloudDeveloperConnectOptions(), cancellationToken);
    }

    /// <summary>
    /// manage connection resources
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> Connections(
        GcloudDeveloperConnectConnectionsOptions options,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineTool(options, cancellationToken);
    }

    /// <summary>
    /// manage Insights Config     resources
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> InsightsConfigs(
        GcloudDeveloperConnectInsightsConfigsOptions options,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineTool(options, cancellationToken);
    }

    /// <summary>
    /// manage Operation resources
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> Operations(
        GcloudDeveloperConnectOperationsOptions options,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineTool(options, cancellationToken);
    }

    #endregion
}
