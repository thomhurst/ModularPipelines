...
public class AzMlFeatureStoreEntity
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMlFeatureStoreEntityCreateOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> List(AzMlFeatureStoreEntityListOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMlFeatureStoreEntityListOptions(), cancellationToken: token);
    }
