...
public class AzSiteRecoveryVaultHealth
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzSiteRecoveryVaultHealthRefreshDefaultOptions(), cancellationToken: token);
    }
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzSiteRecoveryVaultHealthShowOptions(), cancellationToken: token);
    }
}