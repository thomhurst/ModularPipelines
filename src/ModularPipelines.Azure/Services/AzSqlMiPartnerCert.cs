...
public class AzSqlMiPartnerCert
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzSqlMiPartnerCertDeleteOptions(), cancellationToken: token);
    }
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzSqlMiPartnerCertShowOptions(), cancellationToken: token);
    }
}