...
public class AzMeshSecret
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMeshSecretDeleteOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> List(AzMeshSecretListOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMeshSecretListOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Show(AzMeshSecretShowOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMeshSecretShowOptions(), cancellationToken: token);
    }
}