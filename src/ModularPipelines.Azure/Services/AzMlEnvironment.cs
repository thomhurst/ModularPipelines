...
public class AzMlEnvironment
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMlEnvironmentCreateOptions(), cancellationToken: token);
    }
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMlEnvironmentListOptions(), cancellationToken: token);
    }
