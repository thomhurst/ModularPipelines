...
public class AzNetworkcloudRacksku
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzNetworkcloudRackskuListOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Show(AzNetworkcloudRackskuShowOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzNetworkcloudRackskuShowOptions(), cancellationToken: token);
    }
}