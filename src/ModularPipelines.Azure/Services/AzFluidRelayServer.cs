...
public class AzFluidRelayServer
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFluidRelayServerDeleteOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> List(AzFluidRelayServerListOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFluidRelayServerListOptions(), cancellationToken: token);
    }
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFluidRelayServerShowOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Update(AzFluidRelayServerUpdateOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFluidRelayServerUpdateOptions(), cancellationToken: token);
    }
}