...
public class AzSphereDeviceCertificate
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzSphereDeviceCertificateListOptions(), cancellationToken: token);
    }
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzSphereDeviceCertificateShowQuotaOptions(), cancellationToken: token);
    }
}