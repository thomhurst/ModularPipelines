...
public class AzAmsAccountStorage
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzAmsAccountStorageAddOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Remove(AzAmsAccountStorageRemoveOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzAmsAccountStorageRemoveOptions(), cancellationToken: token);
    }
