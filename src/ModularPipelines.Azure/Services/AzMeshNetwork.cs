...
public class AzMeshNetwork
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMeshNetworkDeleteOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> List(AzMeshNetworkListOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMeshNetworkListOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Show(AzMeshNetworkShowOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzMeshNetworkShowOptions(), cancellationToken: token);
    }
}