...
public class AzFzf
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFzfGroupOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Install(AzFzfInstallOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFzfInstallOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Location(AzFzfLocationOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFzfLocationOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Subscription(AzFzfSubscriptionOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzFzfSubscriptionOptions(), cancellationToken: token);
    }
}