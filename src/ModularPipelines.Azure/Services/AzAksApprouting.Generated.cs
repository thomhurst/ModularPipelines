// <auto-generated>
// This file was generated by ModularPipelines.OptionsGenerator.
// Do not edit this file manually.
// </auto-generated>

#nullable enable

using System.CodeDom.Compiler;
using ModularPipelines.Context;
using ModularPipelines.Context.Domains.Shell;
using ModularPipelines.Models;
using ModularPipelines.Options;
using ModularPipelines.Azure.Options;

namespace ModularPipelines.Azure.Services;

/// <summary>
/// az approuting commands.
/// </summary>
[GeneratedCode("ModularPipelines.OptionsGenerator", "2.0.0")]
public class AzAksApprouting
{
    private readonly ICommandContext _command;
    private AzAksApproutingGateway? _gateway;
    private AzAksApproutingZone? _zone;

    /// <summary>
    /// Initializes a new instance of the <see cref="AzAksApprouting"/> class.
    /// </summary>
    public AzAksApprouting(ICommandContext command)
    {
        _command = command;
    }

    #region Sub-command Groups

    /// <summary>
    /// az gateway sub-commands.
    /// </summary>
    public AzAksApproutingGateway Gateway => _gateway ??= new AzAksApproutingGateway(_command);

    /// <summary>
    /// az zone sub-commands.
    /// </summary>
    public AzAksApproutingZone Zone => _zone ??= new AzAksApproutingZone(_command);

    #endregion

    #region Commands

    /// <summary>
    /// Disable App Routing addon.
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="executionOptions">The execution configuration options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> DisableAsync(
        AzAksApproutingDisableOptions? options = null,
        CommandExecutionOptions? executionOptions = null,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineToolAsync(options ?? new AzAksApproutingDisableOptions(), executionOptions, cancellationToken);
    }

    /// <summary>
    /// Enable App Routing.
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="executionOptions">The execution configuration options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> EnableAsync(
        AzAksApproutingEnableOptions? options = null,
        CommandExecutionOptions? executionOptions = null,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineToolAsync(options ?? new AzAksApproutingEnableOptions(), executionOptions, cancellationToken);
    }

    /// <summary>
    /// Update App Routing addon.
    /// </summary>
    /// <param name="options">The command options.</param>
    /// <param name="executionOptions">The execution configuration options.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The command result.</returns>
    public virtual async Task<CommandResult> UpdateAsync(
        AzAksApproutingUpdateOptions? options = null,
        CommandExecutionOptions? executionOptions = null,
        CancellationToken cancellationToken = default)
    {
        return await _command.ExecuteCommandLineToolAsync(options ?? new AzAksApproutingUpdateOptions(), executionOptions, cancellationToken);
    }

    #endregion
}
