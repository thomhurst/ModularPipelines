...
public class AzNetworkDnsDnssecConfig
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzNetworkDnsDnssecConfigDeleteOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Show(AzNetworkDnsDnssecConfigShowOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzNetworkDnsDnssecConfigShowOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> Wait(AzNetworkDnsDnssecConfigWaitOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzNetworkDnsDnssecConfigWaitOptions(), cancellationToken: token);
    }
}