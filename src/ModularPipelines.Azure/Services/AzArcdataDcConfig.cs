...
public class AzArcdataDcConfig
{
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzArcdataDcConfigInitOptions(), cancellationToken: token);
    }

    public async Task<CommandResult> List(AzArcdataDcConfigListOptions? options = default, CancellationToken token = default)
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzArcdataDcConfigListOptions(), cancellationToken: token);
    }
    ...
    {
        return await _command.ExecuteCommandLineTool(options ?? new AzArcdataDcConfigShowOptions(), cancellationToken: token);
    }
}