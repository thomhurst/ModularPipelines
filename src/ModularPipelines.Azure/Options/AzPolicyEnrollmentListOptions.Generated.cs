// <auto-generated>
// This file was generated by ModularPipelines.OptionsGenerator.
// Do not edit this file manually.
// </auto-generated>

#nullable enable

using System.CodeDom.Compiler;
using System.Diagnostics.CodeAnalysis;
using ModularPipelines.Attributes;
using ModularPipelines.Azure.Options;

namespace ModularPipelines.Azure.Options;

/// <summary>
/// Retrieve all applicable policy enrollments.
/// </summary>
[GeneratedCode("ModularPipelines.OptionsGenerator", "")]
[ExcludeFromCodeCoverage]
[CliSubCommand("policy", "enrollment", "list")]
public record AzPolicyEnrollmentListOptions : AzOptions
{
    /// <summary>
    /// Include policy enrollments either inherited from parent scopes or at child scopes.  Allowed values: 0, 1, f, false, n, no, t, true, y, yes.
    /// </summary>
    [CliFlag("--disable-scope-strict-match", ShortForm = "-d")]
    public bool? DisableScopeStrictMatch { get; set; }

    /// <summary>
    /// Filter list results.
    /// </summary>
    [CliFlag("--filter")]
    public bool? Filter { get; set; }

    /// <summary>
    /// The management group.
    /// </summary>
    [CliFlag("--management-group")]
    public bool? ManagementGroup { get; set; }

    /// <summary>
    /// Name of resource group. You can configure the default group using `az configure --defaults group=&lt;name&gt;`.
    /// </summary>
    [CliFlag("--resource-group", ShortForm = "-g")]
    public bool? ResourceGroup { get; set; }

    /// <summary>
    /// Scope at which to list applicable policy enrollments. If scope is not provided, the scope will be the implied or specified subscription.
    /// </summary>
    [CliFlag("--scope")]
    public bool? Scope { get; set; }

}
