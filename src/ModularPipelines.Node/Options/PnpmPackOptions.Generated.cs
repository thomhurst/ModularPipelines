...
[CliSubCommand("pack")]
public record PnpmPackOptions : PnpmOptions
{
    /// <summary>
    ...
    /// <summary>
    /// Directory in which to save the tarball. Defaults to the current working directory
    /// </summary>
    ...
    /// <summary>
    /// Customize the output path. `%s` expands to the package name and `%v` to the version, e.g. `%s.tgz` or `some-dir/%s-%v.tgz`
    /// </summary>
    [CliOption("--out")]
    ...
    /// </summary>
    [CliOption("--pack-gzip-level")]
    public string? PackGzipLevel { get; set; }

    /// <summary>
    /// Keep the original `packageManager` field and publish-lifecycle scripts in the packed manifest instead of stripping them
    /// </summary>
