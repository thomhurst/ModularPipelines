// <auto-generated>
// This file was generated by ModularPipelines.OptionsGenerator.
// Do not edit this file manually.
// </auto-generated>

#nullable enable

using System.CodeDom.Compiler;
using System.Diagnostics.CodeAnalysis;
using ModularPipelines.Attributes;
using ModularPipelines.Eksctl.Options;

namespace ModularPipelines.Eksctl.Options;

/// <summary>
/// Updates the Kubernetes API endpoint access configuration and public access CIDRs.
/// </summary>
[GeneratedCode("ModularPipelines.OptionsGenerator", "2.0.0")]
[ExcludeFromCodeCoverage]
[CliSubCommand("utils", "update-cluster-vpc-config")]
public record EksctlUtilsUpdateClusterVpcConfigOptions : EksctlOptions
{
    /// <summary>
    /// EKS cluster name
    /// </summary>
    [CliOption("--cluster", ShortForm = "-c", Format = OptionFormat.EqualsSeparated)]
    public string? Cluster { get; set; }

    /// <summary>
    /// AWS region. Defaults to the value set in your AWS config (~/.aws/config)
    /// </summary>
    [CliOption("--region", ShortForm = "-r", Format = OptionFormat.EqualsSeparated)]
    public string? Region { get; set; }

    /// <summary>
    /// load configuration from a file (or stdin if set to '-')
    /// </summary>
    [CliOption("--config-file", ShortForm = "-f", Format = OptionFormat.EqualsSeparated)]
    public string? ConfigFile { get; set; }

    /// <summary>
    /// Apply the changes
    /// </summary>
    [CliFlag("--approve")]
    public bool? Approve { get; set; }

    /// <summary>
    /// access for private (VPC) clients
    /// </summary>
    [CliFlag("--private-access")]
    public bool? PrivateAccess { get; set; }

    /// <summary>
    /// access for public clients
    /// </summary>
    [CliFlag("--public-access")]
    public bool? PublicAccess { get; set; }

    /// <summary>
    /// CIDR blocks that EKS uses to create a security group on the public endpoint
    /// </summary>
    [CliOption("--public-access-cidrs", Format = OptionFormat.EqualsSeparated, AllowMultiple = true)]
    public IEnumerable<string>? PublicAccessCidrs { get; set; }

    /// <summary>
    /// Subnet IDs for the control plane
    /// </summary>
    [CliOption("--control-plane-subnet-ids", Format = OptionFormat.EqualsSeparated, AllowMultiple = true)]
    public IEnumerable<string>? ControlPlaneSubnetIds { get; set; }

    /// <summary>
    /// Security group IDs for the control plane
    /// </summary>
    [CliOption("--control-plane-security-group-ids", Format = OptionFormat.EqualsSeparated, AllowMultiple = true)]
    public IEnumerable<string>? ControlPlaneSecurityGroupIds { get; set; }

    /// <summary>
    /// control plane egress mode: AWS_MANAGED or CUSTOMER_ROUTED
    /// </summary>
    [CliOption("--control-plane-egress-mode", Format = OptionFormat.EqualsSeparated)]
    public string? ControlPlaneEgressMode { get; set; }

    /// <summary>
    /// AWS credentials profile to use (defaults to the value of the AWS_PROFILE environment variable)
    /// </summary>
    [CliOption("--profile", ShortForm = "-p", Format = OptionFormat.EqualsSeparated)]
    public string? Profile { get; set; }

    /// <summary>
    /// toggle colorized logs (valid options: true, false, fabulous) (default "true")
    /// </summary>
    [CliOption("--color", ShortForm = "-C", Format = OptionFormat.EqualsSeparated)]
    public string? Color { get; set; }

    /// <summary>
    /// dump logs to disk on failure if set to true
    /// </summary>
    [CliFlag("--dumpLogs", ShortForm = "-d")]
    public bool? Dumplogs { get; set; }

    /// <summary>
    /// help for this command
    /// </summary>
    [CliFlag("--help", ShortForm = "-h")]
    public bool? Help { get; set; }

    /// <summary>
    /// set log level, use 0 to silence, 4 for debugging and 5 for debugging with AWS debug logging (default 3)
    /// </summary>
    [CliOption("--verbose", ShortForm = "-v", Format = OptionFormat.EqualsSeparated)]
    public int? Verbose { get; set; }

}
