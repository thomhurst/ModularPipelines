...
// Source: https://awscli.amazonaws.com/v2/documentation/api/latest/reference/rds/describe-tenant-databases/index.html
...
[CliSubCommand("rds", "describe-tenant-databases")]
public record AwsRdsDescribeTenantDatabasesOptions : AwsOptions
{
    /// <summary>
    /// The user-supplied DB instance identifier, which must match the iden- tifier of an existing instance owned by the Amazon Web Services ac- count. This parameter isn't case-sensitive.
    /// </summary>
    [CliOption("--db-instance-identifier")]
    public string? DbInstanceIdentifier { get; set; }

    /// <summary>
    /// The user-supplied tenant database name, which must match the name of an existing tenant database on the specified DB instance owned by your Amazon Web Services account. This parameter isnt case-sensi- tive.
    /// </summary>
    [CliOption("--tenant-db-name")]
    public string? TenantDbName { get; set; }

    /// <summary>
    /// A filter that specifies one or more database tenants to describe. Supported filters: o tenant-db-name - Tenant database names. The results list only in- cludes information about the tenant databases that match these tenant DB names. o tenant-database-resource-id - Tenant database resource identi- fiers. o dbi-resource-id - DB instance resource identifiers. The results list only includes information about the tenants contained within the DB instances identified by these resource identifiers. (structure) A filter name and value pair that is used to return a more spe- cific list of results from a describe operation. Filters can be used to match a set of resources by specific criteria, such as IDs. The filters supported by a describe operation are docu- mented with the describe operation. NOTE: Currently, wildcards are not supported in filters. The following actions can be filtered: o DescribeDBClusterBacktracks o DescribeDBClusterEndpoints o DescribeDBClusters o DescribeDBInstances o DescribeDBRecommendations o DescribeDBShardGroups o DescribePendingMaintenanceActions Name -&gt; (string) [required] The name of the filter. Filter names are case-sensitive. Values -&gt; (list) [required] One or more filter values. Filter values are case-sensitive. (string) Shorthand Syntax: Name=string,Values=string,string ... JSON Syntax: [ { "Name": "string", "Values": ["string", ...] } ... ]
    /// </summary>
    [CliOption("--filters")]
