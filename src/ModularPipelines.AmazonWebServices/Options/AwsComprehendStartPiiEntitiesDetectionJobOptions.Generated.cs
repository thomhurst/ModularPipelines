...
// Source: https://awscli.amazonaws.com/v2/documentation/api/latest/reference/comprehend/start-pii-entities-detection-job/index.html
...
[CliSubCommand("comprehend", "start-pii-entities-detection-job")]
public record AwsComprehendStartPiiEntitiesDetectionJobOptions : AwsOptions, IValidatableObject
{
    ...
    /// <param name="LanguageCode">The language of the input documents. Enter the language code for English (en) or Spanish (es). Possible values: o en o es o fr o de o it o pt o ar o hi o ja o ko o zh o zh-TW</param>
    public AwsComprehendStartPiiEntitiesDetectionJobOptions(
        string InputDataConfig,
        string OutputDataConfig,
        AwsComprehendStartPiiEntitiesDetectionJobMode Mode,
        string DataAccessRoleArn,
        AwsComprehendStartPiiEntitiesDetectionJobLanguageCode LanguageCode
    )
    {
    ...
    private AwsComprehendStartPiiEntitiesDetectionJobOptions()
    {
        _requiresAlternateInput = true;
    }

    public static AwsComprehendStartPiiEntitiesDetectionJobOptions FromCliInputJson(string cliInputJson)
    {
        global::System.ArgumentException.ThrowIfNullOrWhiteSpace(cliInputJson);
        return new() { CliInputJson = cliInputJson };
    }

    public static AwsComprehendStartPiiEntitiesDetectionJobOptions ForCliSkeleton(string generateCliSkeleton = "input") =>
    ...
    /// <summary>
    /// Provides conguration parameters for the output of PII entity detec- tion jobs. S3Uri -&gt; (string) [required] When you use the OutputDataConfig object with asynchronous oper- ations, you specify the Amazon S3 location where you want to write the output data. The URI must be in the same Region as the API endpoint that you are calling. The location is used as the prefix for the actual location of the output file. When the topic detection job is finished, the service creates an output file in a directory specific to the job. The S3Uri field contains the location of the output file, called output.tar.gz . It is a compressed archive that contains the ouput of the opera- tion. For a PII entity detection job, the output file is plain text, not a compressed archive. The output file name is the same as the input file, with .out appended at the end. Constraints: o max: 1024 o pattern: s3://[a-z0-9][\.\-a-z0-9]{1,61}[a-z0-9](/.*)? KmsKeyId -&gt; (string) ID for the Amazon Web Services Key Management Service (KMS) key that Amazon Comprehend uses to encrypt the output results from an analysis job. Specify the Key Id of a symmetric key, because you cannot use an asymmetric key for uploading data to S3. The KmsKeyId can be one of the following formats: o KMS Key ID: "1234abcd-12ab-34cd-56ef-1234567890ab" o Amazon Resource Name (ARN) of a KMS Key: "arn:aws:kms:us-west-2:111122223333:key/1234abcd-12ab-34cd-56ef-1234567890ab" o KMS Key Alias: "alias/ExampleAlias" o ARN of a KMS Key Alias: "arn:aws:kms:us-west-2:111122223333:alias/ExampleAlias" Constraints: o max: 2048 o pattern: ^\p{ASCII}+$ Shorthand Syntax: S3Uri=string,KmsKeyId=string JSON Syntax: { "S3Uri": "string", "KmsKeyId": "string" }
    /// </summary>
    [CliOption("--output-data-config")]
    public string? OutputDataConfig { get; private init; }

    /// <summary>
    /// Specifies whether the output provides the locations (offsets) of PII entities or a file in which PII entities are redacted. Possible values: o ONLY_REDACTION o ONLY_OFFSETS
    /// </summary>
    [CliOption("--mode")]
    public AwsComprehendStartPiiEntitiesDetectionJobMode? Mode { get; private init; }
    ...
    /// <summary>
    /// The language of the input documents. Enter the language code for English (en) or Spanish (es). Possible values: o en o es o fr o de o it o pt o ar o hi o ja o ko o zh o zh-TW
    /// </summary>
    [CliOption("--language-code")]
    public AwsComprehendStartPiiEntitiesDetectionJobLanguageCode? LanguageCode { get; private init; }

    /// <summary>
    /// Provides configuration parameters for PII entity redaction. This parameter is required if you set the Mode parameter to ONLY_REDACTION . In that case, you must provide a RedactionConfig definition that includes the PiiEntityTypes parameter. PiiEntityTypes -&gt; (list) An array of the types of PII entities that Amazon Comprehend de- tects in the input text for your request. (string) Possible values: o BANK_ACCOUNT_NUMBER o BANK_ROUTING o CREDIT_DEBIT_NUMBER o CREDIT_DEBIT_CVV o CREDIT_DEBIT_EXPIRY o PIN o EMAIL o ADDRESS o NAME o PHONE o SSN o DATE_TIME o PASSPORT_NUMBER o DRIVER_ID o URL o AGE o USERNAME o PASSWORD o AWS_ACCESS_KEY o AWS_SECRET_KEY o IP_ADDRESS o MAC_ADDRESS o ALL o LICENSE_PLATE o VEHICLE_IDENTIFICATION_NUMBER o UK_NATIONAL_INSURANCE_NUMBER o CA_SOCIAL_INSURANCE_NUMBER o US_INDIVIDUAL_TAX_IDENTIFICATION_NUMBER o UK_UNIQUE_TAXPAYER_REFERENCE_NUMBER o IN_PERMANENT_ACCOUNT_NUMBER o IN_NREGA o INTERNATIONAL_BANK_ACCOUNT_NUMBER o SWIFT_CODE o UK_NATIONAL_HEALTH_SERVICE_NUMBER o CA_HEALTH_NUMBER o IN_AADHAAR o IN_VOTER_NUMBER MaskMode -&gt; (string) Specifies whether the PII entity is redacted with the mask char- acter or the entity type. Possible values: o MASK o REPLACE_WITH_PII_ENTITY_TYPE MaskCharacter -&gt; (string) A character that replaces each character in the redacted PII en- tity. Constraints: o min: 1 o max: 1 o pattern: [!@#$%&amp;*] Shorthand Syntax: PiiEntityTypes=string,string,MaskMode=string,MaskCharacter=string JSON Syntax: { "PiiEntityTypes": ["BANK_ACCOUNT_NUMBER"|"BANK_ROUTING"|"CREDIT_DEBIT_NUMBER"|"CREDIT_DEBIT_CVV"|"CREDIT_DEBIT_EXPIRY"|"PIN"|"EMAIL"|"ADDRESS"|"NAME"|"PHONE"|"SSN"|"DATE_TIME"|"PASSPORT_NUMBER"|"DRIVER_ID"|"URL"|"AGE"|"USERNAME"|"PASSWORD"|"AWS_ACCESS_KEY"|"AWS_SECRET_KEY"|"IP_ADDRESS"|"MAC_ADDRESS"|"ALL"|"LICENSE_PLATE"|"VEHICLE_IDENTIFICATION_NUMBER"|"UK_NATIONAL_INSURANCE_NUMBER"|"CA_SOCIAL_INSURANCE_NUMBER"|"US_INDIVIDUAL_TAX_IDENTIFICATION_NUMBER"|"UK_UNIQUE_TAXPAYER_REFERENCE_NUMBER"|"IN_PERMANENT_ACCOUNT_NUMBER"|"IN_NREGA"|"INTERNATIONAL_BANK_ACCOUNT_NUMBER"|"SWIFT_CODE"|"UK_NATIONAL_HEALTH_SERVICE_NUMBER"|"CA_HEALTH_NUMBER"|"IN_AADHAAR"|"IN_VOTER_NUMBER", ...], "MaskMode": "MASK"|"REPLACE_WITH_PII_ENTITY_TYPE", "MaskCharacter": "string" }
    /// </summary>
